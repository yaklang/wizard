...
export const HollowPie: React.FC<HollowPieProps> = (props) => {
    ...
    useEffect(() => {
        ...
        optionRef.current.series[0].color = (newData || []).map(
            (item) => item.color,
        );
    ...
    useEffect(() => {
        if (Array.isArray(data)) {
            ...
            if (Array.isArray(value) && value.length > 0) {
                setDetails({
                    name: value[0]?.name || '',
                    content: (value[0]?.content || '').replaceAll(
                        '<br/>',
                        '\n',
                    ),
                });
            ...
            myChart.on('click', (params) => {
                setDetails({
                    ...
                    // @ts-ignore
                    content: (params?.data?.content || '').replaceAll(
                        '<br/>',
                        '\n',
                    ),
                });
        ...
        <>
            ...
            <div className="echarts-card">
                {dataSource.map((item: any, index: number) => {
                    return (
                        <div className="echarts-card-item" key={index}>
                            <div className="echarts-card-title">
                                {item.key_verbose || item.key}
                            </div>
                            <div className="echarts-card-content">
                                {item.value}
                            </div>
                        </div>
                    );
