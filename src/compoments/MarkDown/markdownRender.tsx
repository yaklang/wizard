          ...
          a: (aProps) => {
            return (
              <a
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  window.open(aProps.href || '', '_blank', 'noopener')
                }}
              />
            )
          },
...
const MERMAID_BTN_CLASS = 'cursor-pointer p-1 text-muted-foreground transition-all hover:text-foreground'
...
export const StreamMarkdown: React.FC<StreamMarkdownProps> = React.memo((props) => {
  ...
  useEffect(() => {
    ...
    const preventPointer = (e: PointerEvent) => {
      ...
      if (t.closest('[data-streamdown="mermaid"]') && !t.closest('button') && !t.closest('[data-mermaid-action]')) {
        e.stopPropagation()
      }
    }
  ...
  return (
    <div ref={wrapperRef} className={classNames('stream-markdown', wrapperClassName)}>
      ...
      >
        <Streamdown
          plugins={{ code, mermaid, math: math as MathPlugin }}
