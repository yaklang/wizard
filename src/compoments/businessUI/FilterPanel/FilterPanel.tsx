...
export const FilterPanel: React.FC<FilterPanelProps> = React.memo((props) => {
    ...
    const onCheck = useMemoizedFn(
        (groupKey: string, data: PluginsSearchData, check: boolean) => {
            const selected = { ...selecteds };
            if (check) {
                selected[groupKey] = [...(selected[groupKey] || []), data];
            } else {
                selected[groupKey] = (selected[groupKey] || []).filter(
