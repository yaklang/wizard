...
export const FilterPanel: React.FC<FilterPanelProps> = React.memo((props) => {
  ...
  useEffect(() => {
    const excludeGroupExtraOptBtn = JSON.stringify(groupList.map((item) => ({ ...item, groupExtraOptBtn: undefined })))
  ...
  const onCheck = useMemoizedFn((groupKey: string, data: PluginsSearchData, check: boolean) => {
    const selected = { ...selecteds }
    if (check) {
