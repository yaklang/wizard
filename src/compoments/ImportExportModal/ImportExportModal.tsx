...
import {
    YakitModal,
    type YakitModalProp,
} from '@/compoments/yakitUI/YakitModal/YakitModal';
 ...
 */
const ImportExportModalInner = <F, R, P>(
    props: ImportExportModalProps<F, R, P>,
) => {
    const {
        getContainer,
        extra,
    ...
    const streamData = useMemo(() => {
        return {
            Progress: progressStream.length
                ? getProgressValue(progressStream[0])
                : 0,
        };
    }, [progressStream.length]);
    ...
    const handleListeners = useMemoizedFn(() => {
        ...
        // ipcRenderer.on(`${token.current}-data`, async (_: any, data: P) => {
        //     importExportStreamRef.current.unshift(data);
        // });
        // ipcRenderer.on(`${token.current}-error`, (_: any, error: any) => {
...
const ImportExportModal = memo(
    <F, R, P>(props: ImportExportModalProps<F, R, P>) => (
