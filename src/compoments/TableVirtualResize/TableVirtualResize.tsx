...
} from './YakitProtoCheckbox/YakitProtoCheckbox';
...
function TableVirtualResizeFunction<T>(
    props: TableVirtualResizeProps<T>,
...
const Table = <T extends any>(props: TableVirtualResizeProps<T>) => {
    ...
    const [columns, setColumns, getColumns] = useGetState<ColumnsTypeProps[]>(
        props.columns,
    ); // 表头
    ...
    const checkboxPropsMap = useCreation(() => {
        ...
        if (getCheckboxProps) {
            data.forEach((record) => {
                const key = (record as Record<string, any>)[renderKey];
    ...
    useHotkeys(
        ...
        () => {
            ...
            for (let i = 0; i < dataLength; i++) {
                if (
                    (data[i] as Record<string, any>)[renderKey] ===
                    (currentRow as Record<string, any>)[renderKey]
                ) {
                    if (i === 0) {
                        index = i;
                        break;
    ...
    const upKey = useDebounceFn(
        (index: number) => {
            ...
            const currentPosition: tablePosition =
                currentDom.getBoundingClientRect();
            const top =
                containerRefPosition.current.top +
                (containerRefPosition.current.height || 0);
            const inViewport =
                currentPosition.top - 28 <= top &&
                currentPosition.top - 28 >= containerRefPosition.current.top;
    ...
    const downKey = useDebounceFn(
        (index: number) => {
            ...
            const currentPosition: tablePosition =
                currentDom.getBoundingClientRect();
            const rowNumber =
                (containerRef.current.clientHeight - 28) / defItemHeight; // 28 表头高度
            ...
            if (!inViewport)
                dom.scrollTop =
                    (index - Math.floor(rowNumber) + y) * defItemHeight + 1 + 6; // 1px border被外圈的border挡住了，所以+1,滚动条边角高度6
        },
        { wait: 200, leading: true },
    ).run;
    useEffect(() => {
        if (isRightClickBatchOperate)
            document.addEventListener('mousedown', handleShiftMousedown);
        return () => {
    ...
    useDeepCompareEffect(() => {
        const newColumns = props.columns.map((ele) => {
            const defColumnItem = columns.find(
                (c) => c.dataKey === ele.dataKey,
            );
            if (!defColumnItem) {
                return ele;
            }
    ...
    // 推拽后重新计算表格宽度
    const recalculatedTableWidth = useMemoizedFn(
        (scrollBarWidth: number, lastColWidth?: boolean) => {
            const cLength = columns.length;
            if (cLength <= 0) return;
            const tWidth: number = columns
                .map((ele) => ele.width || ele.minWidth || 0)
                .reduce((p, c) => p + c);
    ...
    const onChangeCheckbox = useMemoizedFn((checked: boolean) => {
        ...
        if (checked) {
            const keys = data.map((ele, index) =>
                renderKey ? ele[renderKey] : index,
            );
    ...
    const onChangeCheckboxSingle = useMemoizedFn(
        (checked: boolean, key: string, row: T) => {
            if (!rowSelection) return;
    ...
    const onMouseUp = useMemoizedFn((e) => {
        ...
        if (lineStartX.current > lineEndX.current) {
            ...
            const changeWidth =
                (minWidth > width ? minWidth : width) - moveLeftX;
            ...
            if (lineIndex === columns.length - 2) {
                ...
                const lastColumnsMinWidth =
                    columns[columns.length - 1].minWidth || defMinWidth;
                columns[columns.length - 1].width =
                    lastColumnsMinWidth > lastColumnsWidth - moveRightX
                        ? lastColumnsMinWidth
                        : lastColumnsWidth - moveRightX;
            }
            columns[lineIndex].width =
                minWidth > width ? minWidth : width + moveRightX;
    ...
    const onRowClick = useMemoizedFn((record: T, rowIndex: number) => {
        if (preSelectRef.current) {
            ...
            const startKeyIndex = data.findIndex(
                (s) => s[renderKey] === startKey,
            );
    ...
    const onRowContextMenu = useMemoizedFn(
        (record: T, e: React.MouseEvent, rowIndex: number) => {
            if ((selectedRows?.length || 0) > 0) {
                const index = selectedRows.findIndex(
                    (ele) => ele[renderKey] === record[renderKey],
                );
                ...
                if (index === -1) {
                    ...
                    if (props.onRowContextMenu)
                        props.onRowContextMenu(record, [], e);
                    return;
                }
            } else {
                setCurrentIndex && setCurrentIndex(rowIndex);
            ...
            if (props.onRowContextMenu)
                props.onRowContextMenu(record, selectedRows, e);
        },
    );
    ...
    const onSelectSearch = useMemoizedFn(
        (valueSearch: string | string[], colKey: string) => {
            const newFilters = {
                ...filters,
    ...
    const onDateTimeSearch = useMemoizedFn(
        (dates: [Moment, Moment] | null, colKey: string) => {
            let start: number | undefined = undefined;
            let end: number | undefined = undefined;
            if (dates) {
                start = moment(dates[0]).unix();
                end = moment(dates[1]).unix();
                // 如果两者时间戳一致则获取此天的开始与结束作为start/end
                if (start === end) {
                    start = moment(dates[0]).startOf('day').unix();
                    end = moment(dates[1]).endOf('day').unix();
                }
            }
            const newFilters = {
                ...filters,
                [colKey]: dates ? [start, end] : undefined, //给出去的时间 时间戳秒  antd时间组件值要毫秒
                [`${colKey}-time`]: dates
                    ? [moment(dates[0]).valueOf(), moment(dates[1]).valueOf()]
                    : undefined, //antd时间组件显示的时间 时间戳秒  antd时间组件值要毫秒
            };
            setFilters({ ...newFilters });
            // if (props.onChange) props.onChange(1, pagination.limit, sort, newFilters)
        },
     ...
     * @description 选择搜索
     */
    const renderSelect = useMemoizedFn(
        (columnsItem: ColumnsTypeProps, filterKey: string) => {
            return (
                <SelectSearch
    ...
    const renderDatePicker = useMemoizedFn(
        (columnsItem: ColumnsTypeProps, filterKey: string) => {
            return (
                <>
                        ...
                        <RangePicker
                            ...
                            value={
                                filters[`${filterKey}-time`]
                                    ? [
                                          moment(
                                              filters[`${filterKey}-time`][0],
                                          ),
                                          moment(
                                              filters[`${filterKey}-time`][1],
                                          ),
                                      ]
                                    : undefined
                            }
    ...
    useEffect(() => {
        ...
        if (
            popoverKeys.length > 0 &&
            popoverKeys.filter((key) => opensPopover[key]).length === 0
        ) {
            if (onChangTable) onChangTable();
        }
    }, [opensPopover]);
    ...
    const { width: calcTableWidth, height: calcTableHeight } =
        useResizeDetector({
            targetRef: tableRef,
    ...
    useEffect(() => {
        if (!calcTableWidth || !calcTableHeight) return;
        onScrollContainer();
        setWidth(calcTableWidth);
                    ...
                    {renderTitle ? (
                        ...
                        <div
                            className={classNames(
                                styles['virtual-table-heard'],
                            )}
                        >
                            <div
                                className={classNames(
                                    styles['virtual-table-heard-left'],
                                )}
                            >
                                {title && typeof title === 'string' && (
                                    <div
                                        className={classNames(
                                            styles['virtual-table-heard-title'],
                                        )}
                                    >
                                        {title}
                                    </div>
                            ...
                            {extra && React.isValidElement(extra) && (
                                <div
                                    className={classNames(
                                        styles['virtual-table-heard-right'],
                                    )}
                                >
                                    {extra}
                                </div>
            ...
            {(width === 0 && <Spin spinning={true}></Spin>) || (
                ...
                >
                    ...
                    >
                            ...
                            >
                                {columns.map((columnsItem, cIndex) => (
                                    <ColumnsItemRender
                                        ...
                                        scroll={scroll}
                                        rowSelection={
                                            rowSelection as RowSelectionProps<any>
                                        }
                                        cIndex={cIndex}
                        ...
                        <div
                            className={classNames(
                                styles['virtual-table-list-pagination'],
                            )}
                            style={{
                                display:
                                    scroll.scrollBottom <= 10 ? '' : 'none',
                            }}
                        >
                            {loading && !(pagination?.total == data.length) && (
                                <div
                                    className={classNames(
                                        styles['pagination-loading'],
                                    )}
                                >
                                    <LoadingOutlined />
                                </div>
...
const ColumnsItemRender = React.memo((props: ColumnsItemRenderProps) => {
    ...
    const filterKey =
        columnsItem?.filterProps?.filterKey || columnsItem.dataKey;
    const sorterKey =
        columnsItem?.sorterProps?.sorterKey || columnsItem.dataKey;
    ...
    return (
        <div
            ...
            className={classNames(styles['virtual-table-title'], {
                ...
                [styles['virtual-table-row-left']]:
                    columnsItem.align === 'left',
                [styles['virtual-table-row-center']]:
                    columnsItem.align === 'center',
                [styles['virtual-table-row-right']]:
                    columnsItem.align === 'right',
                [styles['virtual-table-title-fixed-right']]:
                    columnsItem.fixed === 'right',
                [styles['virtual-table-title-fixed-right-left-0']]:
                    columnsItem.fixed === 'right' && scroll.scrollRight <= 1,
                [styles['virtual-table-title-fixed-left']]:
        ...
        >
            <div className={classNames(styles['justify-content-between'])}>
                <div className={styles['virtual-title']}>
                    ...
                    {columnsItem.tip && (
                        <Tooltip title={columnsItem.tip}>
                            <QuestionMarkCircleIcon
                                className={styles['icon-question']}
                            />
                        </Tooltip>
                    )}
                </div>
                    ...
                    {columnsItem.sorterProps?.sorter && (
                        <>
                            {disableSorting ? (
                                <div
                                    className={classNames(
                                        styles['virtual-table-sorter-disable'],
                                    )}
                                >
                                    <DisableSorterIcon />
                                </div>
                ...
                {enableDrag &&
                    columnsItem.enableDrag !== false &&
                    cIndex < columns.length - 1 && (
                        <div
                            className={classNames(
                                styles['virtual-table-title-drag'],
                            )}
    ...
    return (
        <div
            className={classNames(styles['virtual-table-row-content'], {
                [styles['virtual-table-row-fixed-left']]:
                    columnsItem.fixed === 'left',
                [styles['virtual-table-row-fixed-left-box-show-none']]:
                    columnsItem.fixed === 'left' && scroll.scrollLeft <= 0,
                [styles['virtual-table-row-fixed-right']]:
                    columnsItem.fixed === 'right',
            ...
            {list.length !== 0 &&
                list.map((item, number) => {
                    return (
                        <React.Fragment
                            key={
                                `${item.data[renderKey]}-${colIndex}` || number
                            }
                        >
                            {(colIndex === 0 && (
                                <CellRenderDrop
                                    ...
                                    onRowClick={() =>
                                        onRowClick(item.data, item.index)
                                    }
                                    onRowDoubleClick={() =>
                                        onRowDoubleClick &&
...
const CellRender = React.memo(
    (props: CellRenderProps) => {
            ...
            if ((selectedRows?.length || 0) > 0) {
                return (
                    selectedRows?.findIndex(
                        (i) => i[renderKey] === item.data[renderKey],
                    ) !== -1
                );
            }
            return false;
        ...
        return (
            <div
                className={classNames(
                    styles['virtual-table-row-cell'],
                    item.data['cellClassName'],
                    {
                        [styles['virtual-table-row-cell-middle']]:
                            size === 'middle',
                        [styles['virtual-table-batch-active-row']]: batchActive,
                        [styles['virtual-table-hover-row']]:
                            mouseCellId === item.data[renderKey],
                        [styles['virtual-table-active-row']]: isSelect,

                        [styles[`virtual-table-cell-${colorTypes}`]]:
                            !!colorTypes,
                        [styles[`virtual-table-hover-cell-${colorTypes}`]]:
                            !!colorTypes &&
                            mouseCellId === item.data[renderKey],
                        [styles[`virtual-table-active-cell-${colorTypes}`]]:
                            !!colorTypes && isSelect,

                        [styles['virtual-table-row-cell-border-right-0']]:
                            isLastItem,
                        [styles['virtual-table-row-cell-border-right-1']]:
                            (batchActive || isSelect) && isLastItem,
                        [styles['virtual-table-row-cell-border-left-1']]:
                            (batchActive || isSelect) && colIndex === 0,
                        [styles['virtual-table-row-cell-disabled']]:
            ...
            >
                ...
                <div
                    className={classNames({
                        [styles['virtual-table-row-ellipsis']]:
                            columnsItem.ellipsis === false ? false : true,
                        [styles['virtual-table-row-no-ellipsis']]:
                            columnsItem.ellipsis === false ? true : false,
                    })}
                >
                    {columnsItem.render
                        ? columnsItem.render(
    ...
    (preProps, nextProps) => {
        ...
        // }
        if (
            preProps.currentRow !== nextProps.currentRow ||
            preProps.selectedRows !== nextProps.selectedRows
        ) {
            return false;
        }
        if (
            preProps.rowSelection?.selectedRowKeys !==
        ...
        return (
            <div
                ...
                className={classNames(
                    ...
                    item.data['cellClassName'],
                    {
                        ...
                        [styles['virtual-table-row-cell-disabled']]:
                            item.data['disabled'] || item.data['Disabled'],
                        [styles['virtual-table-row-cell-move']]:
            ...
            >
                {enableDragSort && isDragging && (
                    <div
                        className={classNames({
                            [styles['virtual-table-row-cell-isDragging']]:
                                isDragging,
                        })}
                        style={{
                            height: 28,
 ...
 * @return {*}
 */
export const TableVirtualResize = React.forwardRef(
    TableVirtualResizeFunction,
) as <T>(
    props: TableVirtualResizeProps<T> & {
...
export const SelectSearch: React.FC<SelectSearchProps> = React.memo((props) => {
    const { originalList, onSelect, value, filterProps, onClose, onQuery } =
        props;
    const {
        filterOptionRender,
        filtersSelectAll,
    ...
    const onSelectSingle = useMemoizedFn(
        (f: string, record?: FiltersItemProps) => {
            onSelect(f, record);
        },
    );
                ...
                {filterSearch && (
                    <div
                        className={classNames(styles['select-search-input'], {
                            [styles['select-search-input-icon']]:
                                filterSearchInputProps.isShowIcon === true,
                        })}
                    >
                        <Input.Search
                            size="small"
                ...
                <div ref={containerRef} className={styles['select-container']}>
                    <div ref={wrapperRef}>
                        {(list.length > 0 &&
                            ...
                            <div className={classNames(styles['no-data'])}>
                                暂无数据
                            </div>
                        )}
                    </div>
        ...
        return (
            <div
                className={classNames(styles['select-search-multiple'], {
                    [styles['select-search-multiple-filterSearch']]:
                        filterSearch,
                })}
            >
                {filterSearch && (
                                ...
                                return (
                                    <div
                                        ...
                                        className={classNames(
                                            styles['select-item'],
                                            {
                                                [styles['select-item-active']]:
                                                    checked,
                                            },
                                        )}
                                        onClick={() =>
                                            onSelectMultiple(item.data)
                                        }
                                    >
                                        <span
                                            className={classNames(
