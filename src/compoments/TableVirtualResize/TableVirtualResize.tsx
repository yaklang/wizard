/* eslint-disable */
// @ts-nocheck
import React, { type ReactNode, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
import {
  useClickAway,
  useCreation,
  useDebounceEffect,
...
import { Divider, Input, Popover, type RadioChangeEvent, Select, Spin, Tooltip } from 'antd'
import { LoadingOutlined } from '@ant-design/icons'
import { useHotkeys } from 'react-hotkeys-hook'
import { useDrag, useDrop, DndProvider } from 'react-dnd'
import { HTML5Backend } from 'react-dnd-html5-backend'
import type { Identifier, XYCoord } from 'dnd-core'
import type { YakitProtoCheckbox, YakitProtoCheckboxProps } from './YakitProtoCheckbox/YakitProtoCheckbox'
...
const Table = <T extends any>(props: TableVirtualResizeProps<T>) => {
  ...
  useHotkeys(
    ...
    () => {
      ...
      for (let i = 0; i < dataLength; i++) {
        if ((data[i] as Record<string, any>)[renderKey] === (currentRow as Record<string, any>)[renderKey]) {
          if (i === 0) {
            index = i
            break
          } else {
  ...
  const upKey = useDebounceFn(
    (index: number) => {
      ...
      const inViewport = currentPosition.top - 28 <= top && currentPosition.top - 28 >= containerRefPosition.current.top
    ...
    if (lineStartX.current > lineEndX.current) {
      ...
      if (lineIndex === columns.length - 2) {
        ...
        columns[columns.length - 1].width =
          lastColumnsMinWidth > lastColumnsWidth - moveRightX ? lastColumnsMinWidth : lastColumnsWidth - moveRightX
      }
      ...
      <>
        <div className={styles['date-time-search']}>
          ...
          <div className={styles['time-rang']}>
            ...
            <YakitTag color="info" onClick={() => onDateTimeSearch([moment().subtract(1, 'day'), moment()], filterKey)}>
              1天
            </YakitTag>
          </div>
        </div>
  ...
  const { width: calcTableWidth, height: calcTableHeight } = useResizeDetector({
    targetRef: tableRef,
  ...
  const { width: calcTableBodyWidth, height: calcTableBodyHeight } = useResizeDetector({
  ...
  return (
    ...
    >
      ...
      {(width === 0 && <Spin spinning={true}></Spin>) || (
        <Spin spinning={isHiddenLoadingUI ? false : loading !== undefined ? loading && pagination?.page == 1 : false}>
          <div
            ref={tableBodyRef}
          ...
          >
              ...
              <div ref={columnsRef} className={classNames(styles['virtual-table-col'])}>
                {columns.map((columnsItem, cIndex) => (
                  <ColumnsItemRender
                    ...
                    isAll={
                      rowSelection?.isAll || (list.length > 0 && rowSelection?.selectedRowKeys?.length === data.length)
                    }
            ...
            <div
              ...
              style={{
                display: scroll.scrollBottom <= 10 ? '' : 'none',
              }}
            >
  ...
  return (
    <div
      ...
      className={classNames(styles['virtual-table-title'], {
        ...
        [styles['virtual-table-title-fixed-right-left-0']]: columnsItem.fixed === 'right' && scroll.scrollRight <= 1,
                ...
                <div
                  className={classNames(styles['virtual-table-filter'], {
                    [styles['virtual-table-filter-value']]: columnsItem.filterProps.filterMultiple
                      ...
                      : filters[filterKey] &&
                        filters[filterKey] !== (columnsItem.filterProps.filtersSelectAll?.textAll || 'all'),
                  })}
                  onClick={() => {
                    setOpensPopover({
                      ...opensPopover,
  ...
  return (
    <div
      className={classNames(styles['virtual-table-row-content'], {
        ...
        [styles['virtual-table-row-fixed-left-box-show-none']]: columnsItem.fixed === 'left' && scroll.scrollLeft <= 0,
        [styles['virtual-table-row-fixed-right']]: columnsItem.fixed === 'right',
        [styles['virtual-table-row-fixed-right-left-0']]: columnsItem.fixed === 'right' && scroll.scrollRight <= 1,
        [styles['virtual-table-row-center']]: columnsItem.align === 'center',
        [styles['virtual-table-row-right']]: columnsItem.align === 'right',
      })}
      style={{
          ...
          <div ref={wrapperRef}>
            {(list.length > 0 &&
              list.map((item) => (
                ...
                >
                  {filterOptionRender ? filterOptionRender(item.data) : item.data.label || item.data.value}
                </div>
