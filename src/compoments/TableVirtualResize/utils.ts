...
export const isCellRedSingleColor = (content?: string) => {
    ...
    try {
        const colors: string[] = content
            .split(' ')
            .filter((item) => !!item && item.indexOf('table-cell-bg-') > -1);
        if (colors.length === 1 && colors[0] === 'table-cell-bg-red')
            return true;
        if (colors.length === 1 && colors[0] === 'table-cell-bg-grey')
            return true;
        else return false;
    } catch (error) {
        return false;
...
// 判断是否是红色/灰色的单色样式
export const getSingleColorType = (
    content?: string,
): 'red' | 'grey' | undefined => {
    if (!content) return;
    ...
    try {
        const colors = content
            .split(' ')
            .filter((item) => !!item && item.includes('table-cell-bg-'));
