...
export const EditTable: React.FC<EditTableProps> = (props) => {
    ...
    useEffect(() => {
        try {
            ...
            if (columnSchema?.items?.properties?.properties) {
                columnSchema.items.properties = columnSchema.items.properties
                    .properties as unknown as PropertiesProps;
            }
            // columnSchema
            const { maxItems = 50, items } = columnSchema;
            ...
            let widthObj: Record<string, number | string> = {};
            newGrid.forEach((item) => {
                widthObj = {
                    ...widthObj,
                    ...(item as Record<string, number | string>),
                };
            });
            ...
            Object.keys(properties).forEach((key) => {
                ...
                newColumns.push({
                    ...
                    render: (text: any) => {
                        ...
                        const keyUiSchema = newUiKeys?.[key];
                        if (
                            keyUiSchema &&
                            !Array.isArray(keyUiSchema) &&
                            keyUiSchema['ui:widget'] === 'textarea'
                        ) {
                            return (
                                <Tooltip title={text}>
                                    <div
    ...
    const isEditing = useMemoizedFn(
        (rowItem: Item) => rowItem._id === editingId,
    );

    const onEdit = useMemoizedFn(
        (record: Partial<Item> & { _id: React.Key }) => {
    ...
    const mergedColumns = realColumns.map((col) => {
        ...
        return {
            ...
            onCell: (record: Item) => ({
                ...
                type: col.type,
                selectOption: (col.enum || []).map((item: any) => ({
                    label: item,
                    value: item,
                })),
