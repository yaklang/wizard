...
import {
    type ColumnSchemaProps,
    EditTable,
    type UiSchemaTableProps,
} from './editTable/EditTable';
...
export const JsonFormWrapper: React.FC<JsonFormWrapperProps> = React.memo(
    (props) => {
        const { jsonSchemaListRef, field, value, schema, uiSchema, disabled } =
            props;

        const [formData, setFormData, getFormData] = useGetState<any>(
            value || {},
        );
                    ...
                    {(enumOptions as any[]).map(
                        (option: any, index: number) => {
                            ...
                            const itemDisabled: any =
                                enumDisabled &&
                                (enumDisabled as string[]).indexOf(
                                    option.value,
                                ) !== -1;
                            return (
                                <YakitCheckbox
            ...
            return (
                <YakitRadioButtons
                    ...
                    options={(enumOptions as object[]).map((option: any) => {
                        const itemDisabled: any =
                            enumDisabled &&
                            (enumDisabled as string[]).indexOf(option.value) !==
                                -1;
                        const info = {
                            value: option.value,
                            label: option.label,
        ...
        return (
            <JsonForm
                ...
                onChange={(e: any) => {
                    ...
                    const isSame =
                        JSON.stringify(updated) === JSON.stringify(e.formData);
                    // 更新表单值
                    setFormData(updated);
                    // 强制刷新
                    if (!isSame) {
