...
const ArrayFieldTitle: React.FC<ArrayFieldTitleProps> = (props: ArrayFieldTitleProps) => {
  const { idSchema, title }: ArrayFieldTitleProps = props
...
const DefaultArrayItem: React.FC<any> = (props) => {
  ...
  const { index, children, disabled, readonly, onDropIndexClick, hasToolbar, hasRemove } = props
  return (
    <div key={index}>
      <div>{children}</div>
      {hasToolbar && (
  ...
  return (
    <div>
      ...
      <div key={`array-item-list-${idSchema.$id}`}>
        {items && items.map((p: any) => DefaultArrayItem(p))}
        {canAdd && (
