...
export const ExportExcel: React.FC<ExportExcelProps> = (props) => {
  ...
  const inBatchExport = (index: number) => {
    ...
    const name = `${fileName}(第${pagination.pagemeta.page}页${exportNumber.current && firstIndx + 1}-${lastIndex})`
  ...
  return (
    <>
      ...
      <Modal title="数据导出" open={visible} onCancel={() => setVisible(false)} footer={null}>
        <Space wrap>
          {Array.from({ length: frequency }).map((_, index) => (
            <Button onClick={() => inBatchExport(index)} key={index}>
