...
const RollingLoadList = <T extends any>(props: RollingLoadListProps<T>) => {
    const {
        data,
        loadMoreData,
        renderRow,
    ...
    const [height] = useListenHeight(containerRef);
    ...
    let originalList = useMemo(() => {
        ...
        for (let index = preLength.current; index < length; index += col) {
            if (index % col === 0) {
                ...
                for (let j = 0; j < col; j++) {
                    if (data[index + j]) {
                        const item: any = data[index + j];
                        indexMapRef.current?.set(
                            `${item[rowKey || 'Id']}`,
                            index + j,
                        );
                        arr.push(item);
                ...
                <div ref={wrapperRef}>
                    {((isGridLayout && col && col > 1) ||
                        ...
                        list.map((i, index) => {
                            ...
                            if (isGridLayout && col && col > 1) {
                                return (
                                    ...
                                    >
                                        {itemArr.map((ele: any) => (
                                            <div
                                                className={`${(col && classNameWidth[col as 2 | 3 | 4]) || ''} ${
                                                    classNameRow || ''
                                                }`}
                                                key={
                                                    ele[rowKey || 'Id'] +
                            ...
                            return itemArr.map((ele: any) => (
                                <div
                                    className={`${(col && classNameWidth[col as 2 | 3 | 4]) || ''} ${classNameRow || ''}`}
                    ...
                    {!loading && !hasMore && (page || 0) > 0 && (
                        <div className="grid-block text-center no-more-text">
                            暂无更多数据
                        </div>
                    )}
                </div>
...
export default RollingLoadList;
