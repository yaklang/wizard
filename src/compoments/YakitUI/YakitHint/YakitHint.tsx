    ...
    const modals = useMemo(() => {
        if (!!modalsPrevious && modalsPrevious.length > 0) {
            for (let index in childModal) {
                if (!Object.prototype.hasOwnProperty.call(childModal, index))
                    continue;
                /** 判断新数据位置是否超出旧数据长度 */
                const position = +Number(index) < modalsPrevious.length;
                if (position) {
                    /** 新数据展示是否为true */
                    const isLatest = childModal[index].content.visible === true;
                    /** 旧数据展示是否为false */
                    const isOld =
                        modalsPrevious[index].content.visible === false;
                    /** 由false变为true时，置顶该弹窗 */
                    if (isLatest && isOld) {
                        setTimeout(() => {
    ...
    return ReactDOM.createPortal(
        <div
            ...
            className={classNames(
                visible
                    ? styles['yakit-hint-wrapper']
                    : styles['yakit-hint-hidden-wrapper'],
...
export const YakitHintWhite: React.FC<YakitHintWhiteProps> = memo((props) => {
    const { getContainer, maskColor, onClose, isMask, visible, ...rest } =
        props;
    const container = useMemo(() => {
        if (!getContainer) return document.body;
        return getContainer;
