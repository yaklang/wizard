...
export const YakitResizeLine: React.FC<YakitResizeLineProps> = (props) => {
    ...
    if (minSize) {
        min = +minSize.toString().split(/px/g)[0]
            ? +minSize.toString().split(/px/g)[0]
            : 100;
    } else {
        min = 100;
    }
    if (maxSize) {
        max = +maxSize.toString().split(/px/g)[0]
            ? +maxSize.toString().split(/px/g)[0]
            : 100;
    } else {
        max = 100;
    }
    ...
    const mouseMove = (event: any) => {
        if (isMove.current) {
            ...
            if (distance[0] <= min || distance[1] <= max) {
                ...
                if (isVer)
                    line.style.transform = `translateY(${start.current + second - first.current}px)`;
                else
                    line.style.transform = `translateX(${start.current + second - first.current}px)`;
    ...
    const resetBox = () => {
        if (isMove.current) {
            ...
            line.style.display = 'none';
            const end = moveLen.current || start.current;
            if (end - start.current !== 0 && !dragResize)
                onMouseUp(end - start.current);
    ...
    useEffect(() => {
        ...
        body.addEventListener('dragend', (e: any) => e.preventDefault());
        return () => {
            if (resize) {
...
export const YakitResizeBox: React.FC<YakitResizeBoxProps> = React.memo(
    (props) => {
        const {
            isVer = false,
            dragResize = false,
        ...
        const [SecondMinSize, setSecondMinSize] = useState<string | number>(
            secondMinSize,
        );
        ...
        const moveSize = useMemoizedFn((size: number) => {
            ...
            const first = firstRef.current;
            const second = secondRef.current;
            const firstSizeNum = isVer
                ? first.clientHeight + size
                : first.clientWidth + size;
            const secondSizeNum = isVer
                ? second.clientHeight - size
                : second.clientWidth - size;
        ...
        const bodyResize = (bodysize?: number) => {
            ...
            const second = secondRef.current;
            const bodySize =
                bodysize || (isVer ? body.clientHeight : body.clientWidth);
            ...
            if (bodySize) {
                ...
                if (isVer) {
                    const firstHeight =
                        (bodySize * firstSize) / (firstSize + secondSize);
                    const secondHeight =
                        (bodySize * secondSize) / (firstSize + secondSize);
        ...
        const moveStart = useMemoizedFn(() => {
            ...
            if (dragResize) {
                const first = firstRef.current;
                const second = secondRef.current;
                dragFirstSize.current = isVer
                    ? first.clientHeight
                    : first.clientWidth;
                dragSecondSize.current = isVer
                    ? second.clientHeight
                    : second.clientWidth;
            }
        });
        ...
        return (
            <div
                ref={bodyRef}
                style={{ ...style, flexFlow: `${isVer ? 'column' : 'row'}` }}
                ...
                {freeze ? (
                    <div
                        ...
                        className={classNames(styles['resize-split-line'], {
                            [styles['resize-split-line-top']]:
                                lineDirection === 'top' && isVer,
                            [styles['resize-split-line-bottom']]:
                                lineDirection === 'bottom' && isVer,
                            [styles['resize-split-line-left']]:
                                lineDirection === 'left' && !isVer,
                            [styles['resize-split-line-right']]:
                                lineDirection === 'right' && !isVer,
                        })}
                    >
                        <div
                            className={classNames({
                                [styles['resize-split-line-in']]:
                                    isShowDefaultLineStyle,
                                [styles['resize-split-line-in-hide']]:
                                    !isShowDefaultLineStyle,
