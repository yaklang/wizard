...
export const NewHTTPPacketEditor: React.FC<NewHTTPPacketEditorProp> = (
    props,
) => {
    const {
        emptyOr,
    ...
    const [hexValue, setHexValue] = useState<Uint8Array>(
        new Uint8Array(StringToUint8Array(originValue)),
    );
    const [monacoEditor, setMonacoEditor] = useState<IMonacoEditor>();
    const [noWordwrap, setNoWordwrap] = useState(false);
    const [popoverVisible, setPopoverVisible] = useState<boolean>(false);
    const [showValue] = useState<string>(originValue);
    // 对比loading
                ...
                extra={
                    !props.noHeader && (
                        ...
                        >
                            {props.extra}
                            {dataCompare &&
                                dataCompare.rightCode.length > 0 && (
                                    <YakitButton
                                        size="small"
                                        type="primary"
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                    ...
                    {mode === 'text' && !empty && (
                        <YakitEditor
                            ...
                            type={
                                props.language || (isResponse ? 'html' : 'http')
                            }
                            value={
                            ...
                            webSocketValue={
                                props.webSocketValue &&
                                new Buffer(props.webSocketValue).toString(
                                    getEncoding(),
                                )
                            }
                            webSocketToServer={
                                props.webSocketToServer &&
                                new Buffer(props.webSocketToServer).toString(
                                    getEncoding(),
                                )
                            }
                    ...
                    {mode === 'hex' && !empty && (
                        <HexEditor
                            className={classNames({
                                [styles['hex-editor-style']]:
                                    props.system === 'Windows_NT',
                            })}
                            showAscii={true}
                            data={hexValue}
                            showRowLabels={true}
                            ...
                            nonce={nonce}
                            onSetValue={
                                props.readOnly ? undefined : handleSetValue
                            }
                        />
                    )}
                </div>
