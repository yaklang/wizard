...
export const YakitMenu: React.FC<YakitMenuProp> = React.memo((props) => {
    ...
    const itemMenuTypeClass = useMemoizedFn(
        (type?: YakitMenuItemProps['type']) => {
    ...
    const generateMenuInfo = useMemoizedFn((data: YakitMenuItemType) => {
        if (
            typeof (data as any as YakitMenuItemDividerProps)['type'] !==
                'undefined' &&
            ...
            if (info.children && info.children.length > 0) {
                const itemInfo: ItemType = {
                    label: (
                        ...
                        >
                            <div
                                className={styles['yakit-submenu-item-content']}
                            >
                                {info.itemIcon}
                                {isHint && !!hintTitle ? (
                                    <Tooltip
                ...
                const itemInfo: ItemType = {
                    label: (
                        <div
                            style={{ minWidth: width }}
                            className={classNames(
                                styles['yakit-menu-item'],
                                itemMenuTypeClass(info.type) || '',
                            )}
                        >
    ...
    let items: any[] = [];
    if (data.length > 0)
        for (let item of data) items.push(generateMenuInfo(item));
