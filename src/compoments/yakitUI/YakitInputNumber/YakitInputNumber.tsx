...
import type {
    ValueType,
    YakitInputNumberHorizontalProps,
    YakitInputNumberProps,
} from './YakitInputNumberType';
    ...
    return (
        <div
            className={classNames(
                ...
                {
                    [styles['yakit-input-number-wrapper-max-large']]:
                        size === 'maxLarge',
                    [styles['yakit-input-number-wrapper-large']]:
                        size === 'large',
                    [styles['yakit-input-number-wrapper-small']]:
                        size === 'small',
                },
                wrapperClassName,
            )}
        >
            {(type === 'horizontal' && (
                <YakitInputNumberHorizontal {...props} />
            )) || (
                <InputNumber
                    {...resProps}
                    onFocus={onFocus}
                    ...
                    className={classNames(
                        ...
                        {
                            [styles['yakit-input-number-max-large']]:
                                size === 'maxLarge',
                            [styles['yakit-input-number-large']]:
                                size === 'large',
                            [styles['yakit-input-number-small']]:
                                size === 'small',
                            [styles['yakit-input-number-focus']]: focus,
                            [styles['yakit-input-number-disabled']]:
                                !!props.disabled,
                        },
                        className,
                    )}
 ...
 */
const YakitInputNumberHorizontal: React.FC<YakitInputNumberHorizontalProps> = (
    props,
) => {
    const { size, step = 1, controls, precision, ...resProps } = props;
    const [value, setValue] = useState<ValueType | null | undefined>(
        props.value,
    );
    const [focus, setFocus] = useState<boolean>(false);
     ...
     * @description: 增加
     */
    const onIncrease = (val: any, step: any) => {
     ...
     * @description: 减少
     */
    const onDecrease = (val: any, step: any) => {
        if (typeof val !== 'number' && val === undefined) return value;
        const precisionFactor = Math.pow(10, precisionRef.current);
        return toPrecision(
            (precisionFactor * val - precisionFactor * step) / precisionFactor,
    ...
    const onAdd = useMemoizedFn(() => {
        ...
        const newVal =
            value || value === 0
                ? onIncrease(Number(value), step) || 0
                : Number(props.min || 0);
        if (newVal && props.max && newVal >= props.max) {
            const precisionFactor = Math.pow(10, precisionRef.current);
            const max = toPrecision(
                (precisionFactor * Number(props.max)) / precisionFactor,
            );
            setValue(max);
    ...
    const onReduce = useMemoizedFn(() => {
        ...
        const newVal =
            value || value === 0
                ? onDecrease(Number(value), step) || 0
                : Number(props.min || 0);
        if (newVal && props.min && newVal <= props.min) {
            const precisionFactor = Math.pow(10, precisionRef.current);
            const min = toPrecision(
                (precisionFactor * Number(props.min)) / precisionFactor,
            );
            setValue(min);
    ...
    const onBlur = useMemoizedFn((e) => {
        setFocus(false);
        const precisionFactor = Math.pow(10, precisionRef.current);
        const newVal = toPrecision(
            (precisionFactor * Number(value)) / precisionFactor,
        );
        setValue(newVal);
        if (props.onChange) props.onChange(newVal);
        if (props.onBlur) props.onBlur(e);
    ...
    return (
        <div
            className={classNames(
                styles['yakit-input-number-horizontal'],
                'yakit-input-number-horizontal-error',
                {
                    ...
                    [styles['yakit-input-number-horizontal-disabled']]:
                        !!props.disabled,
                },
            )}
        >
            ...
            <YakitInputNumber
                ...
                className={classNames(styles['yakit-input-number-wrapper'], {
                    [styles['yakit-input-number-wrapper-disabled']]:
                        !!props.disabled,
                })}
