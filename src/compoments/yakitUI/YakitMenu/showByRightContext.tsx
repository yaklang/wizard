...
const genX = (client: any, coordinate: any, target: any) => {
...
const genY = (client: any, coordinate: any, target: any) => {
 ...
 */
export const showByRightContext = (
    props: YakitMenuProp | ReactNode,
    x?: number,
    y?: number,
    isForce?: boolean,
) => {
    ...
    const div: HTMLDivElement = divExisted
        ? (divExisted as HTMLDivElement)
        : document.createElement('div');

    /** body展示的高度和宽度；表示body在浏览器内显示的区域高度和宽度 */
    const clientHeight = roundDown(
        document.body.getBoundingClientRect().height || 0,
    );
    const clientWidth = roundDown(
    ...
    const render = () => {
        setTimeout(() => {
            ...
            // eslint-disable-next-line react/no-deprecated
            ReactDOM.render(
                <RightContext data={props} callback={offsetPosition} />,
                div,
            );
        });
    };
...
const RightContext: React.FC<RightContextProp> = memo((props) => {
    ...
    useEffect(() => {
        if (
            (wrapperRef.current?.clientWidth || 0) > 0 &&
    ...
    return (
        <div
            className={styles['show-by-right-context-wrapper']}
            ref={wrapperRef}
        >
            {React.isValidElement(data) ? (
                data
            ) : (
                <YakitMenu {...(data as YakitMenuProp)} />
            )}
        </div>
    );
});
