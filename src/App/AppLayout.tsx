...
const AppLayout = () => {
  ...
  const menuSelectedKeys = useMemo(() => {
    const pathNameList = locations.pathname.split('/').filter((it) => it !== '')
    const resultSelectedKey =
      pathNameList.length > 2 ? `/${pathNameList.slice(0, 2).join('/')}` : `/${pathNameList.join('/')}`
    ...
    <Layout hasSider className="h-full text-[14px]">
      ...
      <Layout className="h-full">
            ...
            return index !== headerTitle.length - 1 && item ? (
              ...
              >
                <div className="flex min-w-0 items-center">
                  <div>{item.name}</div>
                  {item.path === YakitRoute.AI_Agent && <AIEngineHeaderControl />}
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <Button type="primary" onClick={() => mcpModalRef.current?.open()}>
                    启用MCP模型
                  </Button>
                </div>
              </div>
        ...
        {permissionsSlice.length > 0 && (
          <Content id="wizard-scroll" className="overflow-y-auto bg-[#FFF]">
            <Outlet />
          </Content>
        )}
      </Layout>
