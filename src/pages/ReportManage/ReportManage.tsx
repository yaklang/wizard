import { deleteProts, getReportTaskGroups, getssetsProts } from '@/apis/reportManage'
    ...
    },
    {
      ...
      render: (value, record) => (
        <>
          ...
          {record.end_time ? ` - ${dayjs.unix(record.end_time).format('YYYY-MM-DD HH:mm')}` : null}
        </>
      ),
    },
    {
      ...
      render: (_, render) => {
        return (
          <ColumnsOperateRender render={render} localRefrech={page.localRefrech} setDeleteValues={setDeleteValues} />
        )
      },
    },
  ]
            ...
            trigger: (
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={deleteValues?.['report_title'].ids?.length ? false : true}
                >
                  批量删除
                </Button>
              </div>
