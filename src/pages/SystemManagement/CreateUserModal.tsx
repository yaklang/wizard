import { postAddUser, postUserReset, putEditUser } from '@/apis/SystemManagementApi'
...
>(({ title, page }, ref) => {
  ...
  const { runAsync: runAsyncEdit, loading: loadingEdit } = useRequest(putEditUser, { manual: true })
    ...
    <WizardModal
      footer={
        <>
          ...
          <Button key="submit" type="primary" onClick={() => onOk()} loading={loading || loadingEdit}>
            确定
          </Button>
        </>
      }
    ...
    >
      <div className="pt-2 px-6">
        <Form form={form} layout="horizontal" {...layout}>
          <Item name="username" label="用户名" rules={[{ required: true, message: '用户名不能为空' }]}>
            <Input placeholder="请输入用户名" disabled={title === '重置密码'} />
          </Item>
          {title === '重置密码' ? (
            <Item name="password" label="密码" initialValue="******">
              <Input
                ...
                suffix={
                  showPassword ? (
                    <Button
                      icon={<CopyOutlined className="cursor-pointer" />}
                      type="link"
                ...
                },
                {
                  validator: (_, value) => {
                    if (typeof value === 'number' && value < 1) {
                      return Promise.reject(new Error('账号有效期最少为一天'))
                    }
                    if (typeof value === 'number' && value > 100 * 10000) {
                      return Promise.reject(new Error('账号有效期最多为一百万天'))
                    }
