...
import type { ContextPressureEchartsProps } from '../../chatTemplate/AIEcharts';
import {
...
import { useRafPolling } from '@/pages/AIAgent/hooks/useRafPolling/useRafPolling';
...
const AIContextToken: FC<{
    ...
    const aiPerfData = useRafPolling({
        ...
        shouldUpdate: (prev, next) => {
            ...
            return (
                ...
                Object.keys(prev.consumption || {}).length !==
                    Object.keys(next.consumption || {}).length
            );
        },
        // 进行数据克隆，确保引用变化
        clone: (data) => ({
    ...
    // 上下文压力集合
    const currentPressuresEcharts: ContextPressureEchartsProps['dataEcharts'] =
        useCreation(() => {
            const data: number[] = [];
            const xAxis: string[] = [];
            ...
            {currentCostEcharts?.data?.length > 0 && (
                <div className={styles['echarts-wrapper']}>
                    <div className={styles['title']}>
                        ...
                        >{`${lastFirstCost < 0 ? '-' : lastFirstCost}ms`}</span>
                    </div>
