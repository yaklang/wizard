...
const AIPlanReviewTree: React.FC<AIPlanReviewTreeProps> = React.memo(
    (props) => {
        const { editable, planReviewTreeKeywordsMap, currentPlansId } = props;
        const [list, setList] = useControllableValue<AIAgentGrpcApi.PlanTask[]>(
            props,
            {
                defaultValue: [],
                valuePropName: 'list',
        ...
        const getMaxChildIndex = (
            ...
            const children = data
                ...
                .map((task) => {
                    ...
                    return parseInt(childIndex.split('-')[0], 10); // 提取第一段数字
                });
        ...
        const onAddSubNode = useMemoizedFn((item: AIAgentGrpcApi.PlanTask) => {
            ...
            // 找到父任务的位置
            const parentPos = list.findIndex(
                (task) => task.index === parentIndex,
            );
         ...
         */
        const getLastSiblingIndex = (
            data: AIAgentGrpcApi.PlanTask[],
            siblingOfIndex: string,
        ): string | null => {
            ...
            const lastSibling = siblings.reduce((prev, current) => {
                const prevNum = parseInt(prev.index.split('-').pop()!, 10);
                const currentNum = parseInt(
                    current.index.split('-').pop()!,
                    10,
                );
        ...
        const onAddBrotherNode = useMemoizedFn(
            (item: AIAgentGrpcApi.PlanTask) => {
                ...
                const lastSiblingIndex = getLastSiblingIndex(
                    list,
                    siblingOfIndex,
                );
                ...
                const newSiblingNum = lastSiblingIndex
                    ? parseInt(lastSiblingIndex.split('-').pop()!, 10) + 1
                    : parseInt(parts[parts.length - 1], 10) + 1;
                const newSiblingIndex = parentIndex
                    ? `${parentIndex}-${newSiblingNum}`
                    : `${newSiblingNum}`;
        ...
        /** 新增节点后，聚焦在该节点上 */
        const onFocusAfterAddNode = useMemoizedFn(
            (item: AIAgentGrpcApi.PlanTask) => {
                setTimeout(() => {
        ...
        const setItem = useMemoizedFn(
            (item: AIAgentGrpcApi.PlanTask, option: SetItemOption) => {
...
const AIPlanReviewTreeItem: React.FC<AIPlanReviewTreeItemProps> = React.memo(
    (props) => {
        const {
            order,
            item,
        ...
        // #region 关联工具及其解释说明
        const extraInfo: AIAgentGrpcApi.PlanReviewRequireExtra | undefined =
            useCreation(() => {
                ...
                <div className={styles['tree-item-content']}>
                    <div
                        ...
                        className={classNames(styles['title'], {
                            [styles['title-editable']]:
                                editable && !item?.isRemove,
                        ...
                        <div className={styles['body']}>
                            ...
                            {description && (
                                <div className={styles['description']}>
                                    <div
                                        className={styles['description-heard']}
                                    >
                                        <SolidAnnotationIcon />
                                        <span>解释</span>
                                    </div>
 ...
 * @param placeholder
 * */
const ContentEditableDiv: React.FC<ContentEditableDivProps> = React.memo(
    (props) => {
...
/** 树的每一项的线 */
const AIPlanReviewTreeLine: React.FC<AIPlanReviewTreeLineProps> = React.memo(
    (props) => {
        ...
        return (
            <div className={styles['prefix-box']}>
                {indexList.map((_: string, idx: number) => {
                    const isEndLine =
                        idx === level - 1 && isLastNode && indexList.length > 1;
                    const left = idx === 0 ? 7 : idx * 16 + 7;
                    const hasBottom =
                        gap &&
                        idx < level - 2 &&
                        idx >= 1 &&
                        idx > nextIndexList.length - 1;
                    return (
                        <div
                            key={idx}
                            // NOTE 样式相关判断勿动，情况复杂
                            className={classNames(styles['vertical-line'], {
                                [styles['no-bulge-line']]:
                                    isEndLine && level !== 1,
                                [styles['slash-last-line']]:
                                    isEndLine && !nextIndex,
                                [styles['slash-right-line']]:
                                    idx === level - 1 && lineType === 'start',
                                [styles['slash-left-line']]:
                                    idx === level - 1 && lineType === 'end',
                                // gap
                                [styles['slash-gap-line-before']]:
                                    gap &&
                                    idx === level - 1 &&
                                    indexList.length > preIndexList.length,
                                [styles['slash-gap-line-after']]:
                                    gap && idx === level - 1,
                                [styles['gap-no-bulge-line']]:
                                    gap &&
                                    idx >= 1 &&
                                    idx >= nextIndexList.length,
                                [styles['gap-left-line']]:
                                    gap &&
                                    ((nextIndexList.length > 0 &&
                                        idx === nextIndexList.length) ||
                                        (nextIndexList.length === 0 &&
                                            idx === 1)),
                            })}
...
/** 树箭头 */
const AIPlanReviewTreeArrowLine: React.FC<AIPlanReviewTreeArrowLineProps> =
    React.memo(() => {
        return (
