...
export const AIForgeInfoOpt: React.FC<AIForgeInfoOptProps> = memo((props) => {
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      <div className={classNames(styles['forge-name'], 'yakit-content-single-ellipsis')} title={info.ForgeName || ''}>
        {info.ForgeName || ''}
      </div>
    ...
    >
      ...
      <div className={styles['forge-form-body']}>
        {isUIParams && params && params.length !== 0 ? (
          ...
          >
            {params && <ExecuteEnterNodeByPluginParams paramsList={params} pluginType="yak" isExecuting={false} />}
          </Form>
        ) : (
