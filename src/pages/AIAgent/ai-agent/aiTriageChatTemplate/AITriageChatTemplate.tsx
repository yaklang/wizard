...
import {
    getValueByType,
    getYakExecutorParam,
} from '@/pages/AIAgent/plugins/editDetails/utils';
...
export const AIForgeInfoOpt: React.FC<AIForgeInfoOptProps> = memo((props) => {
    ...
    const isActive = useMemo(() => {
        ...
        return (
            activeForge.ForgeName === info.ForgeName &&
            activeForge.Id === info.Id
        );
    }, [info, activeForge]);
    ...
    const tags = useMemo(() => {
        ...
        return info?.Tag.map((tag) => {
            return {
                value: tag,
                color: forgeTagColors.current[
                    Math.floor(Math.random() * forgeTagColors.current.length)
                ],
            };
        });
    }, [info?.Tag]);
    ...
    return (
        <div
            className={classNames(styles['forge-info-opt'], {
                [styles['forge-info-active-opt']]: isActive,
            })}
            onClick={handleClick}
        >
            <div
                ...
                <div className={styles['info-footer']}>
                    {tools.length > 0 && (
                        <YakitPopover
                            ...
                            overlayClassName={
                                styles['forge-info-opt-tools-popover']
                            }
                            content={
                                <div
                                    className={styles['tools-popover-content']}
                                >
                        ...
                        >
                            <YakitTag className={styles['footer-tag']}>
                                <div className={styles['tag-body']}>
                                    <SolidToolIcon
                                        className={styles['tool-header-icon']}
                                    />
                                    {tools.length}
...
export const AIForgeForm: React.FC<AIForgeFormProps> = memo((props) => {
    ...
    return (
        <div
            style={{
                maxHeight: wrapperSize ? (wrapperSize.height || 0) - 60 : 240,
            }}
...
export const AIToolForm: React.FC<AIToolFormProps> = memo((props) => {
    ...
    return (
        <div
            style={{
                maxHeight: wrapperSize ? (wrapperSize.height || 0) - 40 : 240,
            }}
            className={styles['ai-tool-form']}
        >
