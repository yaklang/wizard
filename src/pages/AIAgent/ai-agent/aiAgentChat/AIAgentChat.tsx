...
import type { AIAgentChatMode, AIReActTaskChatReviewProps, HandleStartParams } from './type'
import {
  useCreation,
  useDebounceFn,
  useInViewport,
...
import type { AIChatIPCStartParams, ChatIPCSendType, UseTaskChatState } from '@/pages/AIAgent/ai-re-act/hooks/type'
import useChatIPCDispatcher from '../useContext/ChatIPCContent/useDispatcher'
import useChatIPCStore from '../useContext/ChatIPCContent/useStore'
import type { AIAgentGrpcApi, AIInputEvent, AIStartParams } from '@/pages/AIAgent/ai-re-act/hooks/grpcApi'
...
export const AIAgentChat = memo(() => {
  ...
  const handleReleaseReview = useMemoizedFn((_: ChatIPCSendType, id: string) => {
  ...
  const handleSubmitForge = useMemoizedFn((request: AIStartParams, formValue: AIChatIPCStartParams['extraValue']) => {
    ...
    const qs = `我要使用 ${request.ForgeName}forge执行任务${
      request.ForgeParams ? `,参数:${JSON.stringify(request.ForgeParams)}` : `${userQuery ? `,输入${userQuery!}` : ''}`
    }`
    handleStart({
      qs,
      extraValue: {
  ...
  const handleSubmitTool = useMemoizedFn((question: string) => {
    ...
    setMode('re-act')
    handleStart({
      qs: `我要使用 ${activeTool.VerboseName}(${activeTool.Name})工具执行任务${question ? `,输入${question}` : ''}`,
    })
    handleClearActiveTool()
  })
