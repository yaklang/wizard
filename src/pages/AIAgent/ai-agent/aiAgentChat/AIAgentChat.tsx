...
export const AIAgentChat = memo(() => {
    ...
    const onSetKeyTask = useMemoizedFn(() => {
        ...
        setTimeout(() => {
            emiter.emit(
                'switchAIActTab',
                JSON.stringify({ key: AITabsEnum.Task_Content }),
            );
        }, 100);
    });
    ...
    const [
        ...
        { set: setPlanReviewTreeKeywords, reset: resetPlanReviewTreeKeywords },
    ] = useMap<string, AIAgentGrpcApi.PlanReviewRequireExtra>(new Map());
    ...
    const handleShowReviewExtra = useMemoizedFn(
        (info: AIAgentGrpcApi.PlanReviewRequireExtra) => {
    ...
    const handleSendInteractiveMessage = useMemoizedFn(
        (params: AIChatIPCSendParams, type: ChatIPCSendType) => {
    ...
    /** 发送 IsSyncMessage 消息 */
    const handleSendSyncMessage = useMemoizedFn(
        (data: AISendSyncMessageParams) => {
            if (!activeID) return;
    ...
    const handleSendConfigHotpatch = useMemoizedFn(
        (data: AISendConfigHotpatchParams) => {
            if (!activeID) return;
    ...
    const handleTriggerExecForge = useMemoizedFn(
        (forge: AIForge, useForge?: boolean) => {
            if (!forge || !forge.Id) {
    ...
    const handleSubmitForge = useMemoizedFn(
        (
            ...
            formValue: AIChatIPCStartParams['extraValue'],
        ) => {
            setMode('re-act');
    ...
    const handleReplaceActiveForge = useMemoizedFn(
        async (forge: AIForge, useForge?: boolean) => {
            try {
                const forgeID = Number(forge.Id) || 0;
                if (!forgeID) {
                    yakitNotify(
                        'error',
                        `准备使用的模板异常: id('${forgeID}'), 操作失败`,
                    );
                    return;
                }
                ...
                if (!activeForge) setActiveForge(forgeInfo);
                    ...
                    if (forgeInfo.Id === activeForge.Id) {
                        ...
                        let isReplace = false;
                        isReplace =
                            forgeInfo.ForgeName !== activeForge.ForgeName;
                        isReplace = !isEqual(
                            forgeInfo.ParamsUIConfig,
                            activeForge.ParamsUIConfig,
                        );
                        ...
                        if (replaceForgeNoPrompt) {
                            ...
                            if (!replaceForgeNoPromptCache.current)
                                setReplaceShow(true);
                        }
                    }
                }
        ...
        grpcGetAIToolById(toolId)
            .then((res) => {
                ...
                if (!activeTool) setActiveTool(toolInfo);
                    ...
                    if (!replaceToolNoPromptCache.current)
                        setReplaceToolShow(true);
                }
            })
            .catch(() => {});
    });
    ...
    const konwledgeInputStringFn = useMemoizedFn((params: string) => {
        const currentRef =
            mode === 'welcome' ? aiChatWelcomeRef : aiReActChatRef;
        try {
            const data: PageNodeItemProps['pageParamsInfo']['AIRepository'] =
                JSON.parse(params);

            if (
                data?.defualtAIMentionCommandParams &&
                Array.isArray(data.defualtAIMentionCommandParams)
            ) {
            ...
            <YakitHint
                ...
                footerExtra={
                    <YakitCheckbox
                        ...
                        onChange={(e) =>
                            setReplaceForgeNoPrompt(e.target.checked)
                        }
                    >
                        不再提醒
                    </YakitCheckbox>
            ...
            <YakitHint
                ...
                footerExtra={
                    <YakitCheckbox
                        ...
                        onChange={(e) =>
                            setReplaceToolNoPrompt(e.target.checked)
                        }
                    >
                        不再提醒
                    </YakitCheckbox>
...
export const AIReActTaskChatReview: React.FC<AIReActTaskChatReviewProps> =
    React.memo((props) => {
        const {
            reviewInfo,
            planReviewTreeKeywordsMap,
