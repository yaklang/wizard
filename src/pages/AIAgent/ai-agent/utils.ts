 ...
 */
export const reviewListToTrees = (
    ...
    items.forEach((item) => {
        ...
        if (item.isUserAdd && !item.name && !item.goal && !item.tools.length)
            return;
...
export const formatAIAgentSetting = (
    ...
    try {
        data.EnableSystemFileSystemOperator =
            setting.EnableSystemFileSystemOperator ??
            AIAgentSettingDefault.EnableSystemFileSystemOperator;

        data.UseDefaultAIConfig =
            setting.UseDefaultAIConfig ??
            AIAgentSettingDefault.UseDefaultAIConfig;

        data.ForgeName = ''; // 不传
        data.ForgeParams = undefined; // 不传

        data.DisallowRequireForUserPrompt =
            setting.DisallowRequireForUserPrompt ??
            AIAgentSettingDefault.DisallowRequireForUserPrompt;

        data.ReviewPolicy =
            setting.ReviewPolicy || AIAgentSettingDefault.ReviewPolicy;
        ...
        data.DisableToolUse =
            setting.DisableToolUse ?? AIAgentSettingDefault.DisableToolUse;

        data.AICallAutoRetry =
            setting.AICallAutoRetry ?? AIAgentSettingDefault.AICallAutoRetry;

        data.AITransactionRetry =
            setting.AITransactionRetry ??
            AIAgentSettingDefault.AITransactionRetry;

        data.EnableAISearchTool =
            setting.EnableAISearchTool ??
            AIAgentSettingDefault.EnableAISearchTool;
        data.EnableAISearchInternet =
            setting.EnableAISearchInternet ??
            AIAgentSettingDefault.EnableAISearchInternet;
        data.EnableQwenNoThinkMode =
            setting.EnableQwenNoThinkMode ??
            AIAgentSettingDefault.EnableQwenNoThinkMode;
        data.AllowPlanUserInteract =
            setting.AllowPlanUserInteract ??
            AIAgentSettingDefault.AllowPlanUserInteract;

        if (setting?.AllowPlanUserInteract) {
            if (!isNil(setting?.PlanUserInteractMaxCount)) {
                data.PlanUserInteractMaxCount =
                    setting.PlanUserInteractMaxCount || 3;
            } else {
        ...
        if (!isNil(setting?.ReActMaxIteration)) {
            data.ReActMaxIteration =
                setting.ReActMaxIteration ||
                AIAgentSettingDefault.ReActMaxIteration;
        }
        if (!isNil(setting?.TimelineItemLimit)) {
            data.TimelineItemLimit =
                setting.TimelineItemLimit ||
                AIAgentSettingDefault.TimelineItemLimit;
        }
        // TimelineContentSizeLimit 单位是KB，但传到后端需要转换为字节
        data.TimelineContentSizeLimit =
            ((setting.TimelineContentSizeLimit ??
                AIAgentSettingDefault.TimelineContentSizeLimit) as number) *
            1024;

        if (!isNil(setting?.UserInteractLimit)) {
            data.UserInteractLimit =
                setting.UserInteractLimit ||
                AIAgentSettingDefault.UserInteractLimit;
        }
        if (!isNil(setting?.TimelineSessionID)) {
            data.TimelineSessionID =
                setting.TimelineSessionID ||
                AIAgentSettingDefault.TimelineSessionID;
        }
    } catch (error) {}
