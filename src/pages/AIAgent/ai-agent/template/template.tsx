...
import { EditorMilkdownProps } from '@/compoments/MilkdownEditor/MilkdownEditorType';
...
import {
    extractDataWithMilkdown,
    setEditorValue,
} from '../components/aiMilkdownInput/utils';
...
import OpenFileDropdown, {
    OpenFileDropdownItem,
} from '../aiChatWelcome/OpenFileDropdown/OpenFileDropdown';
...
export const AIChatTextarea: React.FC<AIChatTextareaProps> = memo(
    forwardRef((props, ref) => {
                    ...
                    const list = props.footerLeftTypes
                        .map((item) => {
                            let node: FooterLeftTypesComponentProps =
                                {} as FooterLeftTypesComponentProps;
                            if (isString(item)) {
                                switch (item) {
        ...
        const handleSubmit = useMemoizedFn(() => {
            ...
            const { mentions, plainText } = extractDataWithMilkdown(
        ...
        const onFilesChange = useMemoizedFn(
            (files: FileToChatQuestionList[]) => {
        ...
        const onSetMention = useMemoizedFn((params: AIMentionCommandParams) => {
            switch (params.mentionType) {
                case 'focusMode':
                    ...
                    break;
                default:
                    editorMilkdown.current?.action(
                        callCommand<AIMentionCommandParams>(
                            aiMentionCommand.key,
                            params,
                        ),
                    );
                    break;
        ...
        const renderFooterLeftTypes = useMemoizedFn(
            (types: FooterLeftTypesComponentProps[]) => {
                let node: ReactNode[] = [];
                                ...
                                item.component || (
                                    <AIReviewRuleSelect
                                        ...
                                        className={classNames(
                                            styles['review-rule-self-adaptive'],
                            ...
                            node.push(
                                item.component || (
                                    <AIFocusMode
                                        ...
                                        {...item.props}
                                        className={classNames(
                                            styles['focus-mode-self-adaptive'],
                                            item.props?.className,
                                        )}
                                    />
                                ),
        ...
        const onMention = useMemoizedFn(() => {
            editorMilkdown.current?.action(
                callCommand<string>(insertAtCurrentPosition.key, '@'),
            );
        });
        return (
            <div
                className={classNames(
                    styles['ai-chat-textarea'],
                    {
                        ...
                        [styles['ai-review-chat']]:
                            setting.ReviewPolicy === 'ai',
                    },
                    className,
                )}
            ...
            >
                {isHovering && (
                    <div className={styles['drag-hint']}>松开以添加到对话</div>
                )}
                <div
