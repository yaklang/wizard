...
import type { AITaskInfoProps } from '@/pages/AIAgent/ai-re-act/hooks/aiRender';
        ...
        <div className={styles['ai-tree']}>
            {tasks.map((item, index) => {
                ...
                const position = {
                    ...
                    isParentLast:
                        item.level > (next?.level ?? 0) &&
                        (next?.level ?? 0) !== item.level - 1,
        ...
        const [Icon, Card] = useMemo(() => {
            ...
            const contentNode = (
                <div className={styles['node-content']}>
                    <div className={styles['node-content-text']}>
                        {data.goal}
                    </div>
                    <div className={styles['node-content-tag']}>
                        {[
                            data.fail_tool_call_count,
                            data.success_tool_call_count,
                        ].map((item, index) => {
                            if (!item) return null;
            ...
            const style =
                isParentLast && !data.progress ? { marginBottom: '16px' } : {};
            ...
            switch (data.progress) {
                case 'completed':
                    return [
                        <TaskSuccessIcon key="success" />,
                        getWrapper(styles['node-wrapper-success']),
                    ];
                case 'aborted':
                case 'skipped':
                    return [
                        <TaskErrorIcon key="error" />,
                        getWrapper(styles['node-wrapper-error']),
                    ];
                case 'processing':
                    return [
                        <TaskInProgressIcon key="in-progress" />,
                        getWrapper(styles['node-wrapper-in-progress']),
                    ];
                default:
                    return [
                        <div
                            key="circle"
                            className={styles['node-circle-icon']}
                        />,
        ...
        return (
            <div className={styles['ai-tree-node']}>
                {Array.from({ length: lineNum }).map((_, i) => {
                    ...
                    const bgColor =
                        (isLast && isStartOfLevel) || isStart
                            ? 'transparent'
                            : 'var(--Colors-Use-Neutral-Disable)';
                    const height = isEndOfLevel
                        ? lineStyles(i, levelDiff, lineNum)
                        : {};
                    return (
                        <div
                            key={i}
                            className={classNames(styles['node-icon'], {
                                [styles['node-icon-backslash']]:
                                    backslash && i !== 0,
                                [styles['node-icon-parent-slash']]:
                                    i !== 0 && !slash && isParentLast,
