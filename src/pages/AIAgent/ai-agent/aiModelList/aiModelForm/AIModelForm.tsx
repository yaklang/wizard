...
import { NewThirdPartyApplicationConfigBase } from '@/compoments/configNetwork/NewThirdPartyApplicationConfig';
...
export const AIModelForm: React.FC<AIModelFormProps> = React.memo((props) => {
    ...
    const getAIGlobalConfig = useMemoizedFn(() => {
        grpcGetAIGlobalConfig().then((res) => {
            ...
            if (!!item) {
                ...
                // eslint-disable-next-line no-extra-boolean-cast
                const fileName = !!aiModelType
                    ? getFileNameByModelType(aiModelType)
                    : '';
                if (!fileName) return;
                const index = (aiGlobalConfigRef.current[fileName] || [])
                    // eslint-disable-next-line max-nested-callbacks
                    .findIndex((i: AIModelConfig) => isEqualAIModel(i, item));
    ...
    const onAdd = useMemoizedFn(
        (newItem: AIModelConfig, options: AIModelFormAddOptions) => {
    ...
    const onUpdate = useMemoizedFn(
        (newItem: AIModelConfig, options: AIModelFormUpdateOptions) => {
            try {
                if (
                    !aiGlobalConfigRef.current ||
                    currentIndexInConfigRef.current === -1
                )
                    return;
                ...
                if (!fileName) return;
                const oldUpdateItem =
                    aiGlobalConfigRef.current?.[fileName][
                        currentIndexInConfigRef.current
                    ];
                ...
                if (aiModelType !== modelType) {
                    const isHave = newConfig[fileName].find((i) =>
                        isEqualAIModel(i, newUpdateItem),
                    );
                    if (isHave) {
                        yakitNotify(
                            'error',
                    ...
                    newConfig[fileName][currentIndexInConfigRef.current] =
                        newUpdateItem;
                }
    ...
    const onSetAIGlobalConfig = useMemoizedFn(
        (
            config: AIGlobalConfig,
            option: AIModelFormSetAIGlobalConfigOptions,
        ) => {
            setLoading(true);
...
export const AIConfigAPIKeyFormItem: React.FC<AIConfigAPIKeyFormItemProps> =
    React.memo((props) => {
        const { formProps, aiType } = props;
        const [options, setOptions] =
            useState<YakitAutoCompleteProps['options']>();
        const [loading, setLoading] = useState<boolean>(false);
                ...
                if (!!aiType) {
                    ...
                    grpcQueryAIProviderAll(aiType)
                        .then((res) => {
                            ...
                            const options: YakitAutoCompleteProps['options'] =
                                // eslint-disable-next-line max-nested-callbacks
