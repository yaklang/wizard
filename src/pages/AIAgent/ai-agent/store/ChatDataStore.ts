...
import type { AIChatQSData, ReActChatBaseInfo } from '@/pages/AIAgent/ai-re-act/hooks/aiRender'
...
export class ChatDataStore {
  ...
  /** 创建新的聊天数据 */
  public create(session: string): AIChatData {
  ...
  /** 判断指定聊天数据是否存在 */
  public has(session: string): boolean {
    return this.map.has(session)
  }

  /** 获取指定聊天数据 */
  public get(session: string): AIChatData | undefined {
    return this.map.get(session)
  }

  /** 获取会话聊天列表的数据 */
  public getContentMap({ session, chatType, mapKey }: GetContentMapParams): AIChatQSData | undefined {
  ...
  /** 设置指定聊天数据 */
  public set(session: string, value: SetStateAction<AIChatData>): void {
   ...
   */
  public updater(session: string, updateData: DeepPartial<AIChatData>): void {
    const prev = this.map.get(session)
    if (!prev) {
      throw new Error(`Session: ${session} does not exist`)
    }
      ...
      if (updateData.hasOwnProperty(key)) {
        ...
        if (getExactType(updateValue) === 'Object' && !!updateValue) {
          ...
          if (
            key in result &&
            // @ts-ignore
            getExactType(result[key]) === 'Object'
          ) {
            // @ts-ignore
  ...
  /** 删除指定聊天数据 */
  public remove(session: string): void {
