...
import {
    KnowledgeBaseItem,
    useKnowledgeBase,
} from '@/pages/KnowledgeBase/hooks/useKnowledgeBase';
...
const KnowledgeSidebarList = (
    { api, streams }: KnowledgeSidebarListProps,
    ...
    const [selectedKnowledgeBaseItems, setSelectedKnowledgeBaseItems] =
        useSafeState<KnowledgeBaseTableHeaderProps['knowledgeBaseItems']>(
        ...
        },
        {
            onSuccess: (result) => {
                const resultList = targetInstallList
                    .map((name) =>
                        result.find(
                            (it) => it.Name === name && !it.InstallPath,
                        ),
                    )
                    .filter((v) => v !== undefined);
                const exclude = [
                ...
                const filteredInstall = resultList.filter(
                    (item) => !exclude.includes(item.Name),
                );
                if (filteredInstall.length !== 0) {
                    info(
                        `使用知识库缺少第三方依赖，需安装${filteredInstall.length}个`,
                    );
                    setInstallPlug(true);
                } else {
    ...
    const starKnowledgeeBaseEntry = useMemoizedFn(
        async (updateItems: KnowledgeBaseItem) => {
            try {
                ...
                if (api && typeof api.createStream === 'function') {
                    api.createStream(updateItems.streamToken, {
                        ...
                        onEnd: () => {
                            api.removeStream &&
                                api.removeStream(updateItems.streamToken);
    ...
    const processed = useMemoizedFn(() => {
        const isExternal = (it) =>
            it.IsImported === true && (it.CreatedFromUI ?? false) === false;
        const isManual = (it) =>
            it.IsImported === false && it.CreatedFromUI === true;
        const isOther = (it) =>
            it.IsImported === false && (it.CreatedFromUI ?? false) === false;

        const result =
            addMode.length === 0
                ? knowledgeBases
                : knowledgeBases.filter((it) => {
                      if (addMode.includes('external') && isExternal(it))
                          return true;
                      if (addMode.includes('manual') && isManual(it))
                          return true;
    ...
    useDebounceEffect(
        () => {
            ...
            if (search.trim() === '') {
                ...
                const filtered = processed().filter((kb) =>
                    kb.KnowledgeBaseName.includes(search.trim()),
                );
            ...
            {installPlug ? (
                ...
                <div className={styles['knowledge-base-info-body']}>
                    ...
                    <div className={styles['knowledge-base-info-list']}>
                        <YakitSpin
                            wrapperClassName={
                                styles['knowledge-base-info-list-spin']
                            }
                            spinning={loading}
                        >
                            {knowledgeBase.length > 0 ? (
                                <>
                                    ...
                                    <div className={styles['min-reached']}>
                                        已经到底啦~
                                    </div>
                                </>
                            ) : (
...
export default forwardRef<KnowledgeModalRef, KnowledgeSidebarListProps>(
