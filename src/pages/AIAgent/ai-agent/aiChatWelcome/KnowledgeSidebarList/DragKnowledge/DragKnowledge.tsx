...
const DragKnowledge = () => {
    const { knowledgeBases, addKnowledgeBase, editKnowledgeBase } =
        useKnowledgeBase();
    ...
    const beforeUploadFun = useDebounceFn(
        async (fileList: Array<File & { path: string }>) => {
            ...
            const findDefaultKnowledgeBase = knowledgeBases.find(
                (it) => it.IsDefault,
            );
            ...
            try {
                await runAsync({
                    ...
                    KnowledgeBaseName:
                        findDefaultKnowledgeBase?.KnowledgeBaseName ??
                        'default',
                    ...
                    CreatedFromUI:
                        findDefaultKnowledgeBase?.CreatedFromUI ?? true,
                });
            } catch (error) {
    ...
    const { runAsync, loading } = useRequest(
        async (params) => {
            ...
            const hasKnowledgeBaseById = knowledgeBases.find(
                (it) => it.ID === KnowledgeBaseID,
            );
                    ...
                    <div className={styles['upload-info']}>
                        ...
                        <div className={styles['content']}>
                            <div className={styles['title']}>
                                可将文件拖入框内，或
                                <span className={styles['hight-light']}>
                                    点击此处导入
                                </span>
                                即可开始创建知识库
                            </div>
