...
const AIChatWelcome: React.FC<AIChatWelcomeProps> = React.memo(
    forwardRef((props, ref) => {
        ...
        const [randomAIMaterials, setRandomAIMaterials] =
            useState<GetRandomAIMaterialsResponse>();
        ...
        const onKnowledgeBaseMore = useMemoizedFn(() => {
            emiter.emit(
                'menuOpenPage',
                JSON.stringify({ route: YakitRoute.AI_REPOSITORY }),
            );
        });
        ...
        const onCheckItem = useMemoizedFn(
            (item: AIRecommendItemProps['item']) => {
        ...
        const randomAIMaterialsData: RandomAIMaterialsDataProps =
            useCreation(() => {
                ...
                const knowledgeBases: AIMaterialsData = {
                    type: '知识库',
                    data: (randomAIMaterials?.KnowledgeBaseEntries || []).map(
                        (knowledgeBase) => ({
                            type: '知识库',
                            name:
                                knowledgeBase.KnowledgeTitle ||
                    ...
                    <div className={styles['content-absolute']}>
                        <div className={styles['input-wrapper']}>
                            ...
                            {checkItems.length > 0 ? (
                                <div
                                    className={
                                        styles['suggestion-tips-wrapper']
                                    }
                                >
                                    <div
                                        className={
                                            styles['suggestion-tips-title']
                                        }
                                    >
                                        <span>你可能想问:</span>
                                        {/* {loading ? (
                        ...
                        {!isEmptyAIMaterials && (
                            <div className={styles['recommend-wrapper']}>
                                <AIDownAngleLeftIcon
                                    className={styles['recommend-down-left']}
                                />
                                <AIDownAngleRightIcon
                                    className={styles['recommend-down-right']}
                                />
                                <AIUpAngleLeftIcon
                                    className={styles['recommend-up-left']}
                                />
                                <AIUpAngleRightIcon
                                    className={styles['recommend-up-right']}
                                />
                                ...
                                <YakitSpin spinning={loadingAIMaterials}>
                                    <div className={styles['recommend-body']}>
                                        {Object.keys(randomAIMaterialsData).map(
                                            (key) => {
                                                const aiItem: AIMaterialsData =
                                                    randomAIMaterialsData[
                                                        key as keyof typeof randomAIMaterialsData
                                                    ];
                                                return aiItem.data.length >
                                                    0 ? (
                                                    <AIRecommend
                                                        icon={aiItem.icon}
                                                        hoverIcon={
...
export const SideSettingButton: React.FC<SideSettingButtonProps> = React.memo(
    (props) => {
        ...
        const onSideSetting = useDebounceFn(
            useMemoizedFn((e) => {
                ...
                setRemoteValue(
                    RemoteAIAgentGV.AIAgentSideShowMode,
                    `${checked}`,
                );
...
const AIRecommendItem: React.FC<AIRecommendItemProps> = React.memo((props) => {
    ...
    useEffect(() => {
        ...
        const svgWidth =
            Math.abs(linePointRight.left - linePointLeft.right) - 4;
    ...
    const lineIcon = useCreation(() => {
        return (
            ...
            >
                <path
                    d={generatePath()}
                    stroke="var(--Colors-Use-Neutral-Border)"
                    strokeLinecap="round"
                />
            </svg>
        );
