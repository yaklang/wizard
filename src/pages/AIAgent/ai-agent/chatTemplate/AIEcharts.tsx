...
export const ContextPressureEcharts: React.FC<ContextPressureEchartsProps> =
    React.memo((props) => {
        const { dataEcharts, threshold } = props;
        const [option, setOption] = useState<EChartsOption>(
            getContextPressureOption(dataEcharts, threshold),
        );
        useUpdateEffect(() => {
            onSetOption();
        ...
        const onSetOption = useDebounceFn(
            () => {
                const newOption = getContextPressureOption(
                    dataEcharts,
                    threshold,
                );
                setOption(newOption);
        ...
        return (
            <ReactECharts option={option} style={{ width: 72, height: 24 }} />
        );
    });
const color = {
    low: {
...
export const ResponseSpeedEcharts: React.FC<ResponseSpeedEchartsProps> =
    React.memo((props) => {
        const { dataEcharts } = props;
        const [option, setOption] = useState<EChartsOption>(
            getResponseSpeedOption(dataEcharts),
        );
        useUpdateEffect(() => {
            onSetOption();
        }, [dataEcharts]);
