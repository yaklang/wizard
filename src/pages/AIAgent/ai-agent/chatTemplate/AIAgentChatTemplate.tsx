...
import {
    ...
    type AITaskStartInfo,
    type ReActChatRenderItem,
} from '@/pages/AIAgent/ai-re-act/hooks/aiRender';
import type { AIEventQueryRequest } from '@/pages/AIAgent/ai-re-act/hooks/grpcApi';
...
import { type Components, type ItemProps, Virtuoso } from 'react-virtuoso';
import useVirtuosoAutoScroll from '@/pages/AIAgent/ai-re-act/hooks/useVirtuosoAutoScroll';
...
export const AIChatLeftSide: React.FC<AIChatLeftSideProps> = memo((props) => {
    ...
    return (
        <div
            className={classNames(styles['ai-chat-left-side'], {
                [styles['ai-chat-left-side-hidden']]: !expand,
            })}
        >
            <YakitResizeBox
                isVer
...
/** @name chat-信息流展示 */
export const AIAgentChatStream: React.FC<AIAgentChatStreamProps> = memo(
    (props) => {
        ...
        const components = useMemo<Components<ReActChatRenderItem>>(
            () => ({
                Item,
                Footer,
            }),
        ...
        const onScrollToIndex = useMemoizedFn((id) => {
            const index = streams.findIndex((item) => {
                if (item.type === AIChatQSDataTypeEnum.TASK_INDEX_NODE) {
                    ...
                    const taskIndex = (chatItem.data as AITaskStartInfo)
                        .taskIndex;
                    return taskIndex === id;
                }
                return false;
            ...
            <div className={styles['ai-agent-chat-stream']}>
                <Virtuoso<ReActChatRenderItem>
                    ...
                    initialTopMostItemIndex={
                        streams.length > 1 ? streams.length - 1 : 0
                    }
...
export const AIChatToolDrawerContent: React.FC<AIChatToolDrawerContentProps> =
    memo((props) => {
                        ...
                        {toolList.map((info) => {
                            ...
                            switch (type) {
                                ...
                                case AIChatQSDataTypeEnum.TOOL_CALL_RESULT: {
                                    ...
                                    const fileList =
                                        execFileRecord.get(CallToolID);
                                    const nodeLabel = NodeIdVerbose['Zh'];
                                    return (
                                        <StreamCard
                                            key={id}
                                            titleText={nodeLabel}
                                            titleIcon={
                                                taskAnswerToIconMap[NodeId]
                                            }
                                            content={
