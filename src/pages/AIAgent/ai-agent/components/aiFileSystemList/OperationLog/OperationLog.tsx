...
import {
    getFileActionStatus,
    isPluginExecuteLogFileItem,
} from '@/pages/AIAgent/utils/invoker';
import classNames from 'classnames';
import { formatTime } from '@/utils/timeUtil';
import { YakitEmpty } from '@/compoments/YakitUI/YakitEmpty/YakitEmpty';
import moment from 'moment';
import { useMemoizedFn } from 'ahooks';
import type { PluginExecuteLogFile } from '@/pages/AIAgent/plugins/operator/pluginExecuteResult/PluginExecuteResultType';
...
const VirtuosoListContainer = forwardRef<HTMLDivElement, ListProps>(
    ({ children, style, ...props }, ref) => {
        return (
            <div
                {...props}
                ref={ref}
                style={style}
                className={styles['virtuoso-item-list']}
            >
                {children}
            </div>
        );
...
const FilePreview: FC<FilePreviewProps> = ({ level, data, content }) => {
    ...
    const { is_dir, file_size, description, path } =
        data as FileLogShowDataProps;
    return (
        <span>
            <div>
            ...
            {description && (
                <div className={styles['file-description']}>{description}</div>
            )}
...
const OperationLog: FC<OperationLogProps> = ({ loading, list }) => {
    ...
    const components = useMemo(
        () => ({
            ...
            Footer: () =>
                list.length > 0 ? <div className={styles['arrow']} /> : null,
        }),
        [list.length],
    );
    ...
    return (
        <div className={styles['log-wrapper']}>
            {!loading && list.length === 0 ? (
                <YakitEmpty style={{ paddingTop: 48 }} title="暂无操作日志" />
            ) : (
