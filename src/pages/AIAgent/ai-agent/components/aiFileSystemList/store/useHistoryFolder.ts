import {
    bindExternalStoreHook,
    createExternalStore,
} from './createExternalStore';
    ...
    try {
        ...
        if (saved) {
            ...
            if (Array.isArray(parsed)) {
                store.setSnapshot(() =>
                    parsed.filter((item) => item.path).slice(-RECENT_COUNT),
                );
            }
        }
    } catch (e) {
    ...
    async addHistoryItem({ path, isFolder }: HistoryItem) {
        store.setSnapshot((prevList) => {
            ...
            const filtered = prevList.filter(
                (item) => item.path.trim() !== '' && item.path !== trimmedPath,
            );
            const newItem: HistoryItem = { path, isFolder };
            const existingIndex = prevList.findIndex(
                (item) => item.path === trimmedPath,
            );
            let nextList;
            if (existingIndex !== -1) {
                nextList = [...filtered];
            ...
            const finalResult =
                nextList.length > RECENT_COUNT
                    ? nextList.slice(-RECENT_COUNT)
                    : nextList;
            setRemoteValue(REMOTE_KEY, JSON.stringify(finalResult)).catch(
                console.error,
            );
    ...
    removeHistoryItem(path: string) {
        store.setSnapshot((prevList) => {
            ...
            if (nextList.length !== prevList.length) {
                setRemoteValue(REMOTE_KEY, JSON.stringify(nextList)).catch(
                    console.error,
                );
            }
