...
import {
    ...
    aiMentionFactory,
    AIMilkdownMention,
} from './aiMilkdownMention/AIMilkdownMention';
...
import { placeholderConfig, placeholderPlugin } from '@/compoments/Placeholder';

const remarkDirective = $remark(`remark-directive`, () => directive);

export const AIMilkdownInputBase: React.FC<AIMilkdownInputBaseProps> =
    React.memo(
        React.forwardRef((props, ref) => {
            ...
            useEffect(() => {
                ...
                editor?.action((ctx) => {
                    // 简单阻止所有文件粘贴
                    ctx.get(editorViewCtx).dom.addEventListener(
                        'paste',
                        (e) => {
            ...
            const onSetMention = useMemoizedFn(
                (params: AIMentionCommandParams) => {
                    get()?.action(
