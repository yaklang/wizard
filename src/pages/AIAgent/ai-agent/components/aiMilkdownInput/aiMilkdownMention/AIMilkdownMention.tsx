...
import { iconMap } from '@/pages/AIAgent/ai-agent/defaultConstant'
...
import type { AIMentionCommandParams } from './aiMentionPlugin'
...
export const AIMilkdownMention: React.FC<AIMilkdownMentionProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const syncMentionVisible = () => {
      setMentionVisible(div.dataset.show === 'true' || div.getAttribute('aria-expanded') === 'true')
    }
    const observer = new MutationObserver(syncMentionVisible)

    syncMentionVisible()
    observer.observe(div, {
      attributes: true,
      attributeFilter: ['data-show', 'aria-expanded'],
    })
  ...
  return (
    <div
      ...
      ref={ref}
    >
      {mentionVisible && <AIChatMention onSelect={onSure} filterMode={filterMode} />}
    </div>
  )
}
...
export const AICustomMention: React.FC<AICustomMentionProps> = () => {
