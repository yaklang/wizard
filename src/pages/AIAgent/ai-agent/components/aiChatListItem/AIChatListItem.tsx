...
import type { AIChatMentionSelectItem } from '../aiChatMention/type';
...
export const isHaveFreeDialogFileList = (
    extraValue: HandleStartParams['extraValue'],
): FileToChatQuestionList[] => {
    if (
        has(extraValue, 'freeDialogFileList') &&
        isArray(extraValue.freeDialogFileList)
    ) {
...
export const isHaveSelectForges = (
    extraValue: HandleStartParams['extraValue'],
): AIChatMentionSelectItem[] => {
...
export const isHaveSelectTools = (
    extraValue: HandleStartParams['extraValue'],
): AIChatMentionSelectItem[] => {
...
export const isHaveSelectKnowledgeBases = (
    extraValue: HandleStartParams['extraValue'],
): AIChatMentionSelectItem[] => {
    if (
        has(extraValue, 'selectKnowledgeBases') &&
        isArray(extraValue.selectKnowledgeBases)
    ) {
...
export const AIChatListItem: React.FC<AIChatListItemProps> = React.memo(
    (props) => {
        ...
        const getTask = useMemoizedFn((id) =>
            taskChat.plan.find((item: any) => item.index === id),
        );
                    ...
                    return (
                        <AITriageChatContent
                            ...
                            contentClassName={classNames({
                                [styles['file-content-wrapper']]:
                                    isExtraShow(extraValue),
                            })}
                        />
                    );
        ...
        return (
            <React.Fragment key={item.token}>{renderContent()}</React.Fragment>
        );
    },
);
