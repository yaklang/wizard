...
export const AIChatMention: React.FC<AIChatMentionProps> = React.memo(
    (props) => {
        const { onSelect, defaultActiveTab, filterMode } = props;
        const [activeKey, setActiveKey, getActiveKey] =
            useGetSetState<AIMentionTabsEnum>(
        ...
        const onLeftArrow = useDebounceFn(
            () => {
                ...
                if (index >= 0 && index < mentionTabs.length) {
                    newValue =
                        (mentionTabs[index - 1]?.value as AIMentionTabsEnum) ||
                        getActiveKey();
                }
        ...
        const mentionTabs = useCreation(() => {
            ...
            if (filterMode?.length) {
                tabs = tabs.filter(
                    (item) =>
                        !filterMode.includes(
                            item.value as `${AIMentionTabsEnum}`,
                        ),
                );
            }
...
const ForgeNameListOfMention: React.FC<ForgeNameListOfMentionProps> =
    React.memo(
        forwardRef((props, ref) => {
            ...
            const onKeyboardSelect = useMemoizedFn(
                (value: number, isScroll: boolean) => {
            ...
            const getList = useMemoizedFn(async (page?: number) => {
                ...
                try {
                    ...
                    const length =
                        newPage === 1
                            ? res.Data.length
                            : res.Data.length + response.Data.length;
            ...
            return (
                <div
                    className={styles['forge-name-list-of-mention']}
                    ref={forgeListRef}
                    tabIndex={0}
                >
        ...
        const getList = useMemoizedFn(async (page?: number) => {
            ...
            try {
                ...
                const length =
                    newPage === 1
                        ? res.Tools.length
                        : res.Tools.length + response.Tools.length;
                setHasMore(length < Number(res.Total));
...
const AIMentionSelectItem: React.FC<AIMentionSelectItemProps> = React.memo(
    (props) => {
...
const FileSystemTreeOfMention: React.FC<FileSystemTreeOfMentionProps> =
    React.memo(() => {
...
const FocusModeOfMention: React.FC<FocusModeOfMentionProps> = React.memo(
    forwardRef((props, ref) => {
        ...
        return (
            <div
                className={styles['focus-mode-list-of-mention']}
                ref={toolListRef}
            >
                <YakitSpin spinning={spinning}>
