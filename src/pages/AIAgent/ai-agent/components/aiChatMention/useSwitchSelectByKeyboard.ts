...
function useSwitchSelectByKeyboard<T>(
    ...
    const { data, selected, rowKey, onSelectNumber, onEnter, getContainer } =
        params;
    ...
    const onUpArrow = useDebounceFn(
        () => {
            ...
            const currentIndex = data.findIndex(
                (item) => getRowKey(item) === currentRowKey,
            );
            if (currentIndex === -1) return;
            const newScrollToNumber = currentIndex - 1;
            const id = data[newScrollToNumber]
                ? getRowKey(data[newScrollToNumber])
                : '';
            if (!id) return;
    ...
    const isVisible = useMemoizedFn(
        (dom: HTMLElement, callback: (b: boolean) => void) => {
