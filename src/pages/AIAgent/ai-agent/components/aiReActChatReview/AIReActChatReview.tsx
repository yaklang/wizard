...
import type {
    AIReActChatReviewProps,
    ForgeReviewFormProps,
    ForgeReviewFormRefProps,
} from './AIReActChatReviewType';
...
import AIPlanReviewTree from '@/pages/AIAgent/ai-agent/aiPlanReviewTree/AIPlanReviewTree';
import { handleFlatAITree } from '../../../ai-re-act/hooks/utils';
import { reviewListToTrees } from '@/pages/AIAgent/ai-agent/utils';
import { grpcGetAIForge } from '@/pages/AIAgent/ai-agent/grpc';
...
import type { AIChatIPCSendParams } from '@/pages/AIAgent/ai-agent/useContext/ChatIPCContent/ChatIPCContent';
import {
    OutlineHandleColorsIcon,
...
export const AIReActChatReview: React.FC<AIReActChatReviewProps> = React.memo(
    (props) => {
        const {
        ...
        const [reviewTreeOption, setReviewTreeOption] =
            useState<AIAgentGrpcApi.ReviewSelector>();
        const [reviewTrees, setReviewTrees] = useState<
            AIAgentGrpcApi.PlanTask[]
        >([]);
        const [currentPlansId, setCurrentPlansId] = useState<string>('');
        const [forgeOption, setForgeOption] =
            useState<AIAgentGrpcApi.ReviewSelector>();
        const forgeReviewFormRef = useRef<ForgeReviewFormRefProps>({
            validateFields: () => {},
        });
        ...
        useEffect(() => {
            switch (type) {
                ...
                case 'require_user_interactive':
                    // eslint-disable-next-line no-case-declarations
                    const { options } =
                        review as AIAgentGrpcApi.AIReviewRequire;
                    if (options && options.length > 0) {
                        const value =
                            options[0].prompt || options[0].prompt_title;
        ...
        const toolReview = useCreation(() => {
            ...
            const { tool, tool_description, params } =
                review as AIAgentGrpcApi.ToolUseReviewRequire;
            let paramsValue = '-';
            try {
            ...
            return (
                <div className={styles['review-task-tool-data']}>
                    ...
                    <div className={styles['tool-params']}>
                        ...
                        <div className={styles['params-content']}>
                            {paramsValue}
                        </div>
                    </div>
                </div>
        ...
        const forgeReview = useCreation(() => {
            ...
            return (
                <ForgeReviewForm
                    ref={forgeReviewFormRef}
                    editable={!forgeOption}
                    {...data}
                />
            );
        }, [review, forgeOption]);
                    ...
                    <div className={styles['review-task-tool-data']}>
                        ...
                        <div className={styles['task-summary']}>
                            ...
                            <div className={styles['summary-detail']}>
                                <YakitPopover
                                    overlayClassName={
                                        styles['task-review-summary-popover']
                                    }
                                    content={
                                        <div
                                            className={
                                                styles['task-long-summary']
                                            }
                                        >
                                            <div
                                                className={
        ...
        const planReview = useCreation(() => {
            if (reviewTrees.length > 0) {
                const list = reviewTreeOption
                    ? reviewTrees
                    : initReviewTreesRef.current;
                return (
                    <AIPlanReviewTree
        ...
        const handleCallbackEdit = useMemoizedFn((cb: boolean) => {
            if (cb && editInfo.current) {
                ...
                if (allow_extra_prompt && reviewQS)
                    jsonInput.extra_prompt = reviewQS;
        ...
        const handleContinue = useMemoizedFn(() => {
            ...
            const find = (
                (review as AIAgentGrpcApi.ToolUseReviewRequire)?.selectors || []
            ).find((item) => item.value === 'continue');
            if (!find) return;
            const { value } = find;
            const jsonInput: Record<string, string> = { suggestion: value };
            onSendAIByValue(JSON.stringify(jsonInput), value);
        ...
        const handleShowEdit = useMemoizedFn(
            (info: AIAgentGrpcApi.ReviewSelector) => {
                switch (info.value) {
                    case 'freedom-review':
        ...
        const isContinue = useCreation(() => {
            ...
            const findIndex = (
                review as AIAgentGrpcApi.ToolUseReviewRequire
            ).selectors.findIndex((item) => item.value === 'continue');
            return findIndex !== -1;
        }, [review]);
        const onSendAIByValue = useMemoizedFn(
                ...
                if (reviewTreeOption) {
                    return (
                        <>
                            <YakitButton
                                type="outline2"
                                onClick={() => setReviewTreeOption(undefined)}
                            >
                                取消
                            </YakitButton>
                            <YakitButton
                                type="primary"
                                onClick={handleSubmitReviewTree}
                            >
                                提交
                            </YakitButton>
                        </>
                ...
                if (forgeOption) {
                    return (
                        <>
                            <YakitButton
                                type="outline2"
                                onClick={() => setForgeOption(undefined)}
                            >
                                取消
                            </YakitButton>
                            <YakitButton
                                type="primary"
                                onClick={handleSubmitForge}
                            >
                                提交表单内容
                            </YakitButton>
                        </>
                    ...
                    <>
                        {!!noAIOptionsList.showButton.length && (
                            <div
                                className={
                                    styles[
                                        'review-selectors-showButton-wrapper'
                                    ]
                                }
                            >
            ...
            return (
                <div className={styles['btn-group']}>
                    ...
                    {type === 'require_user_interactive' && (
                        <YakitButton
                            disabled={!isRequireQS}
                            loading={requireLoading}
                            onClick={handleAIRequireQSSend}
                        >
                            提交
                        </YakitButton>
                    )}
        ...
        const reviewHeardExtra = useCreation(() => {
            ...
            switch (type) {
                ...
                case 'exec_aiforge_review_require':
                    /** NOTE 定义问题 */
                    // eslint-disable-next-line no-case-declarations
                    const toolReviewData =
                        review as AIAgentGrpcApi.ToolUseReviewRequire;
                    if (toolReviewData.aiReview) {
                        const { interactive_id, score, level } =
                            toolReviewData.aiReview;
                        node = (
                            <>
                                {!!interactive_id && !score && !countdown && (
                                    <div>评估中...</div>
                                )}
                                {!!score && (
                                    <div>
                                        ...
                                        <span
                                            className={classNames(
                                                styles['ai-countdown'],
                                                {
                                                    [styles['ai-score-low']]:
                                                        level === 'low',
                                                    [styles['ai-score-middle']]:
                                                        level === 'middle',
                                                    [styles['ai-score-high']]:
                                                        level === 'high',
                                                },
                                            )}
                                        >
            ...
            <>
                ...
                >
                    <div className={styles['review-header']}>
                        <div className={styles['review-header-title']}>
                            <div className={styles['title-style']}>
                                <OutlineHandleColorsIcon
                                    className={styles['hand-colors-icon']}
                                />
                                {reviewTitle.title}
                            </div>
                        ...
                        <div className={styles['review-header-extra']}>
                            {reviewHeardExtra}
                        </div>
                    </div>
                    <div className={styles['review-container']}>
                        ...
                        {!reviewTreeOption &&
                        (aiOptions || noAIOptionsAllowShowInput) ? (
...
const ForgeReviewForm: React.FC<ForgeReviewFormProps> = React.memo(
    forwardRef((props, ref) => {
        ...
        const params: YakParamProps[] = useCreation(() => {
            ...
            try {
                const param: YakParamProps[] =
                    handleFetchParams(ParamsUIConfig);
                return param;
            } catch (error) {
                return [];
            }
