...
function useYakExecResult(
    params?: UseYakExecResultParams,
): [UseYakExecResultState, UseYakExecResultEvents];
...
function useYakExecResult(params?: UseYakExecResultParams) {
    ...
    // card
    const cardKVPair = useRef<Map<string, AIAgentGrpcApi.AICacheCard>>(
        new Map<string, AIAgentGrpcApi.AICacheCard>(),
    );
    const cardTimeRef = useRef<NodeJS.Timeout | null>(null);
    const [card, setCard] = useState<AIAgentGrpcApi.AIInfoCard[]>([]);
    const [yakExecResultLogs, setYakExecResultLogs] = useState<
        StreamResult.Log[]
    >([]); // log

    const execFileRecordOrder = useRef(1);
    /** 插件执行过程中的文件操作记录 */
    const [execFileRecord, setExecFileRecord] = useThrottleState<
        Map<string, AIYakExecFileRecord[]>
    >(new Map(), {
        wait: 2000,
    });
    const updateExecFileRecord = useMemoizedFn(
        (CallToolID: string, info: StreamResult.Log) => {
            try {
                setExecFileRecord((old) => {
                    ...
                    record.push({
                        ...info,
                        id: uuidv4(),
                        order: execFileRecordOrder.current++,
                    });
    ...
    const onHandleCard = useMemoizedFn(
        (value: AIAgentGrpcApi.AICardMessage) => {
            const logData = value.content as StreamResult.Log;
            const checkInfo: AIAgentGrpcApi.AICard = checkStreamValidity(
                value.content as StreamResult.Log,
            );
            if (!checkInfo) return;
    ...
    const onSetCard = useMemoizedFn(() => {
        ...
        cardTimeRef.current = setTimeout(() => {
            const cacheCard: AIAgentGrpcApi.AIInfoCard[] = convertCardInfo(
                cardKVPair.current,
            );
            setCard(() => [...cacheCard]);
    ...
    const onHandleYakExecResult = useMemoizedFn(
        (CallToolID: string, value: AIAgentGrpcApi.AIPluginExecResult) => {
            try {
                ...
                const obj: AIAgentGrpcApi.AICardMessage = JSON.parse(
    ...
    const onAIStatusHandleCard = useMemoizedFn(
        (info: { key: string; value: string; timestamp: number }) => {
        ...
        try {
            ...
            if (res.Type === 'yak_exec_result') {
                const data = JSON.parse(
                    ipcContent,
                ) as AIAgentGrpcApi.AIPluginExecResult;
    ...
    const handleSetYakResult = useMemoizedFn(
        (newData: UseYakExecResultState) => {
