...
): [UseTaskChatState, UseTaskChatEvents];
...
function useTaskChat(params?: UseTaskChatParams) {
    ...
    const [elements, setElements, getElements] = useGetSetState<
        ReActChatRenderItem[]
    >([]);
    const handleSetElements = useMemoizedFn(
        (newElements: ReActChatRenderItem[]) => {
    ...
    const setContentMap = useMemoizedFn(
        (mapKey: string, value: AIChatQSData) => {
            const contentMap = getChatDataStore?.()?.taskChat?.contents;
    ...
    /** 任务节点开始执行, 生成UI展示的信息 */
    const handleTaskStartNode = useMemoizedFn(
        (res: AIOutputEvent, nodeInfo: AIAgentGrpcApi.ChangeTask) => {
            try {
                // 任务树根节点不进行节点展示
    ...
    const handleUpdateTaskState = useMemoizedFn(
        (index: string, state: AITaskInfoProps['progress']) => {
            setPlan((old) => {
                return old.map((item) => {
    ...
    const handlePlanReview = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            const isAuto = isAutoExecuteReviewContinue({
                type: res.Type,
                getFunc: getRequest,
            });
            const chatData: AIChatQSData = {
                ...
                data: {
                    ...cloneDeep(data),
                    selected: isAuto
                        ? JSON.stringify({ suggestion: 'continue' })
                        : undefined,
    ...
    const handlePlanReviewExtra = useMemoizedFn((res: AIOutputEvent) => {
        try {
            if (
                !review.current ||
                review.current.type !== AIChatQSDataTypeEnum.PLAN_REVIEW_REQUIRE
            ) {
                handlePushLog(
                    genErrorLogData(
                        res.Timestamp,
                        `${res.Type}数据异常: 未找到对应plan_review_require数据`,
                    ),
                );
                return;
            }
    ...
    const handleReviewJudgement = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            score.levelLabel =
                AIReviewJudgeLevelMap[score?.level || '']?.label || undefined;
            const info = review.current.data;
            if (
                !info.aiReview ||
                (info.aiReview && typeof info.aiReview.seconds === 'undefined')
            ) {
    ...
    const handleSpecialData = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            if (res.Type === 'plan') {
                // 更新正在执行的任务树
                const tasks = JSON.parse(ipcContent) as {
                    root_task: AIAgentGrpcApi.PlanTask;
                };
    ...
    /** review 界面选项触发事件 */
    const handleSend: handleSendFunc = useMemoizedFn(
        ({ request, optionValue, cb }) => {
            try {
                ...
                (chatData.data as AIReviewType).selected =
