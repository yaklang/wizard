...
function useCasualChat(
    params?: UseCasualChatParams,
): [UseCasualChatState, UseCasualChatEvents];

function useCasualChat(params?: UseCasualChatParams) {
    const { pushLog, getChatDataStore, getRequest, onReviewRelease } =
        params || {};
    ...
    const handleUserRequireReview = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            setElements((old) => {
                ...
                newArr.push({
                    token: review.current.id,
                    type: review.current.type,
                    renderNum: 1,
                    chatType: 'reAct',
                });
                return newArr;
        ...
        try {
            ...
            if (
                ...
                if (
                    ...
                    (review.current.data.aiReview &&
                        typeof review.current.data.aiReview.seconds ===
                            'undefined')
                ) {
            ...
            if (
                ...
                if (
                    ...
                    (chatData.data.aiReview &&
                        typeof chatData.data.aiReview.seconds === 'undefined')
                ) {
    ...
    const handleSend: handleSendFunc = useMemoizedFn(
        ({ request, optionValue, extraValue, cb }) => {
            try {
                const {
                ...
                if (IsInteractiveMessage && InteractiveId) {
                    if (
                        !review.current ||
                        (review.current.data as AIReviewType)?.id !==
                            InteractiveId
                    ) {
                    ...
                    (review.current.data as AIReviewType).selected =
                        request.InteractiveJSONInput;
                    (review.current.data as AIReviewType).optionValue =
                        optionValue;
