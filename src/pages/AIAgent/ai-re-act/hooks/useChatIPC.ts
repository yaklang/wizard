...
import { useEventSource, useGetSetState } from '@/hooks';
...
function useChatIPC(params?: UseChatIPCParams) {
    ...
    const sendFirstMessage = useMemoizedFn(
        (session: string, params: AIInputEvent) => {
            // @ts-ignore
            postSendFirstMessage(session, params);
            setTimeout(() => {
    ...
    const sseEvents = useEventSource(`run/${chatID.current}/events`, {
        maxRetries: 1,
        manual: true,
        isAIAgent: true,
    ...
    const onSwitchChat = useMemoizedFn((session?: string) => {
        ...
        if (execute) {
            ...
            postCancelMessage(chatID.current).catch(() => {});
        } else {
            endAfterSession.current = '';
