...
function useChatIPC(params?: UseChatIPCParams) {
    ...
    const handleQuestionQueueStatusChange = useMemoizedFn(
        (res: AIOutputEvent) => {
            try {
                ...
                if (
                    NodeId === 'react_task_dequeue' &&
                    data.hasOwnProperty('focus_mode')
                ) {
                    // 记录专注模式状态
    ...
    const onStart = useMemoizedFn((args: AIChatIPCStartParams) => {
        ...
        ipcRenderer.on(`${token}-data`, (_: any, res: AIOutputEvent) => {
            try {
                ...
                // 记录会话中所有的RunTimeID
                setRunTimeIDs((old) => {
                    if (!res.CallToolID || old.includes(res.CallToolID))
                        return old;
                ...
                if (
                    res.Type === 'structured' &&
                    res.NodeId === 'session_title'
                ) {
                    // 生成会话的名称
                    const nameInfo = JSON.parse(ipcContent) as {
                        title: string;
                    };
                ...
                if (res.Type === 'start_plan_and_execution') {
                    ...
                    if (
                        ...
                        if (casualChatID.current === taskChatID.current)
                            handleResetCasualChatLoading();
                        // 标记grpc流里属于任务规划的流
                        planCoordinatorId.current = startInfo.coordinator_id;
                        ...
                        const newMemoryEntryList: AIAgentGrpcApi.MemoryEntryList =
                            {
                                ...
                                memory_pool_limit:
                                    Number(
                                        taskMemorys.current.memory_pool_limit,
                                    ) +
                                    Number(
                                        reactMemorys.current.memory_pool_limit,
                                    ),
                                memory_session_id:
                                ...
                                score_overview: {
                                    A_total:
                                        Number(
                                            taskMemorys.current.score_overview
                                                .A_total,
                                        ) +
                                        Number(
                                            reactMemorys.current.score_overview
                                                .A_total,
                                        ),
                                    C_total:
                                        Number(
                ...
                if (res.Type === 'structured') {
                    ...
                    if (obj?.level) {
                        ...
                        if (planCoordinatorId.current === res.CoordinatorId)
                            return;
                        /* 问题的状态变化 */
                        const { react_task_id, react_task_now_status } =
                            JSON.parse(
                                ipcContent,
                        ...
                        if (data.key === 're-act-loading-status-key') {
                            if (
                                planCoordinatorId.current === res.CoordinatorId
                            ) {
                                // 任务规划-loading展示标题
                                setTaskStatus((old) => {
                                    if (old.loading) {
                ...
                if (res.Type === 'stream') {
                    if (res.IsSystem || res.IsReason) {
                        ...
                        let ipcStreamDelta =
                            Uint8ArrayToString(StreamDelta) || '';
                        ...
                        // 输出实时系统信息流
                        if (res.IsSystem)
                            handleSetSystemStream(EventUUID, content);
                        return;
                    }
        ...
        ipcRenderer.on(`${token}-end`, (_: any, res: any) => {
            console.log('end', res);
            saveStateDataOfEnd();
        ...
        ipcRenderer.on(`${token}-error`, (_: any, err: any) => {
    ...
    const onSwitchChat = useMemoizedFn((session?: string) => {
        ...
        if (execute) {
            ...
            ipcRenderer
                .invoke('cancel-ai-re-act', chatID.current)
                .catch(() => {});
        } else {
            endAfterSession.current = '';
