...
function useFileTree(
    params: UseFileTreeParams,
): [UseFileTreeState, UseFileTreeEvents];
...
function useFileTree(params: UseFileTreeParams) {
    ...
    const getParentNode = useMemoizedFn((parentPath: string) => {
        try {
            ...
            if (treeData.current.path === parentPath)
                return cloneDeep(treeData.current);
    ...
    // 外界触发的调整文件树数据
    const onTriggerUpdateTreeData = useMemoizedFn(
        async (list: FileMonitorItemProps[]) => {
            for (let ev of list) {
    ...
    const startPolling = useMemoizedFn(() => {
        ...
        pollingTimer.current = setInterval(() => {
            ...
            // 将数组前三个裁剪出来
            const foldersToFetch = pendingFolderList.current.splice(
                0,
                availableSlots,
            );
            ...
            try {
                ...
                if (pendingFolderList.current.includes(folderPath)) {
                    pendingFolderList.current =
                        pendingFolderList.current.filter(
                            (item) => item !== folderPath,
                        );
                }
