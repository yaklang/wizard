...
import type { AIChatQSData, AIToolResult, ReActChatGroupElement } from './aiRender'
...
function useChatContent(params: UseChatContentParams) {
  const { chatType, getContentMap, setContentMap, getElements, setElements, pushLog, handleUnkData } = params
  ...
  const handleInitStream = useMemoizedFn((res: AIOutputEvent) => {
    try {
      ...
      if (CallToolID) {
        ...
        !idFind &&
          callToolIDToUUIDMap.current.set(CallToolID, ids.concat([{ eventUUID: event_writer_id, nodeID: NodeId }]))
      }
  ...
  const handleStream = useMemoizedFn((res: AIOutputEvent) => {
    try {
      ...
      const content = (base64ToJson(res.Content) || '') + (base64ToJson(res.StreamDelta) || '')
      ...
      if (!streamData || streamData.type !== AIChatQSDataTypeEnum.STREAM) {
        pushLog(genErrorLogData(res.Timestamp, `异常 stream 类型, NodeId: ${NodeId}, eventuuid: (${EventUUID})`))
        return
      }
      ...
      if (CallToolID) {
        ...
        !idFind && callToolIDToUUIDMap.current.set(CallToolID, ids.concat([{ eventUUID: EventUUID, nodeID: NodeId }]))
      }
