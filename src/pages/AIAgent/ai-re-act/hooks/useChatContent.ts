...
import type { UseChatContentEvents, UseChatContentParams } from './type';
import {
    AIStreamContentType,
...
function useChatContent(params: UseChatContentParams) {
    ...
    const updateElements = useMemoizedFn(
        (
            main: { mapKey: string; type: AIChatQSDataTypeEnum },
            sub?: { mapKey: string; type: AIChatQSDataTypeEnum },
        ) => {
            // 先判断该项是否存在
            const target = getElements().findIndex(
                (item) =>
                    item.token === main.mapKey &&
                    item.type === main.type &&
                    (sub ? item.isGroup : true),
            );
            try {
                if (target >= 0) {
                    ...
                    const subIndex = item.children.findIndex(
                        (item) =>
                            item.token === sub.mapKey && item.type === sub.type,
                    );
                    if (subIndex >= 0) {
     ...
     */
    const callToolIDToUUIDMap = useRef<
        Map<string, { eventUUID: string; nodeID: string }[]>
    >(new Map());
     ...
     */
    const toolResultErrorUUIDToCallToolID = useRef<Map<string, string>>(
        new Map(),
    );
    /**
     ...
     */
    const streamToToolResultError = useRef<
        Map<string, { content: string; uuid: string; status: 'start' | 'end' }>
    >(new Map());
    ...
    const handleIsGroupDisplay = useMemoizedFn(
        (params: {
            ...
            if (
                contentType !== AIStreamContentType.DEFAULT ||
                renderList.length === 0
            ) {
            ...
            if (lastRender.token === mapKey) {
                ...
                updateElements(
                    { mapKey, type: lastRender.type },
                    lastRender.isGroup ? { mapKey, type } : undefined,
                );
                return;
            }
            ...
            if (
                !lastRenderData ||
                lastRenderData.type !== AIChatQSDataTypeEnum.STREAM
            ) {
                updateElements({ mapKey, type });
                return;
            }
            ...
            if (
                lastRender.type === AIChatQSDataTypeEnum.STREAM &&
                !lastRender.isGroup
            ) {
                // 单项的stream数据
                if (lastRenderData.data.NodeId === nodeID) {
                    ...
                    for (let el of arr) {
                        ...
                        if (info)
                            setContentMap(el, {
                                ...info,
                                parentGroupKey: lastRender.token,
                            });
                    }
                    setElements((old) => {
                        const newArr = [...old];
                ...
                lastRender.type === AIChatQSDataTypeEnum.STREAM_GROUP &&
                lastRender.isGroup
            ) {
                // 组的stream数据
                if (lastRenderData.data.NodeId === nodeID) {
                    ...
                    if (subData) {
                        setContentMap(mapKey, {
                            ...subData,
                            parentGroupKey: lastRender.token,
                        });
                    }
                    updateElements(
                        { mapKey: lastRender.token, type: lastRender.type },
                        { mapKey: mapKey, type: type },
                    );
                } else {
                    // 未命中
                    updateElements({ mapKey, type });
        ...
        try {
            ...
            if (!event_writer_id) {
                pushLog(
                    genErrorLogData(
                        res.Timestamp,
                        `${res.Type}数据(NodeId: ${NodeId}), event_writer_id 为空`,
                    ),
                );
                return;
            }
            ...
            if (isToolStdoutStream(NodeId) && !!CallToolID) {
                ...
                setContentMap(event_writer_id, {
                    ...
                    data: {
                        NodeId,
                        NodeIdVerbose:
                            res.NodeIdVerbose || convertNodeIdToVerbose(NodeId),
            ...
            if (isToolStderrStream(NodeId) && !!CallToolID) {
                if (
                    !toolResultErrorUUIDToCallToolID.current.has(
                        event_writer_id,
                    )
                ) {
                    toolResultErrorUUIDToCallToolID.current.set(
                        event_writer_id,
                        CallToolID,
                    );
                }
            ...
            if (CallToolID) {
                ...
                const idFind = ids.find(
                    (item) =>
                        item.eventUUID === event_writer_id &&
                        item.nodeID === NodeId,
                );
                !idFind &&
                    callToolIDToUUIDMap.current.set(
                        CallToolID,
                        ids.concat([
                            { eventUUID: event_writer_id, nodeID: NodeId },
                        ]),
                    );
            }
    ...
    const handleStream = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            if (!!CallToolID && isToolStderrStream(NodeId)) {
                const call_tool_id =
                    toolResultErrorUUIDToCallToolID.current.get(EventUUID);
                const errorResult = streamToToolResultError.current.get(
                    call_tool_id || '',
                );
            ...
            if (!!CallToolID && isToolStdoutStream(NodeId)) {
                ...
                const toolForStreamData = getContentMap(
                    toolResult.data.stream.EventUUID,
                );
                if (
                    !toolForStreamData ||
                    toolForStreamData.type !== AIChatQSDataTypeEnum.STREAM
                ) {
                    pushLog(
                        genErrorLogData(
                            res.Timestamp,
                            `EventUUID: ${toolResult.data.stream.EventUUID} 的stream数据没有对应的初始化`,
                        ),
                    );
                    return;
                }
            ...
            if (
                !streamData ||
                streamData.type !== AIChatQSDataTypeEnum.STREAM
            ) {
                pushLog(
                    genErrorLogData(
                        res.Timestamp,
                        `异常 stream 类型, NodeId: ${NodeId}, eventuuid: (${EventUUID})`,
                    ),
                );
                return;
            }
            ...
            if (CallToolID) {
                ...
                const idFind = ids.find(
                    (item) =>
                        item.eventUUID === EventUUID && item.nodeID === NodeId,
                );
                !idFind &&
                    callToolIDToUUIDMap.current.set(
                        CallToolID,
                        ids.concat([{ eventUUID: EventUUID, nodeID: NodeId }]),
                    );
            }
        ...
        try {
            ...
            if (!event_writer_id) {
                pushLog(
                    genErrorLogData(
                        res.Timestamp,
                        `stream-finished数据, event_writer_id 为空`,
                    ),
                );
                return;
            }
            ...
            if (isToolStdoutStream(node_id)) {
                ...
                toolResult.data.tool.toolStdoutContent = {
            ...
            if (streamData.parentGroupKey) {
                updateElements(
                    {
                        mapKey: streamData.parentGroupKey,
                        type: AIChatQSDataTypeEnum.STREAM_GROUP,
                    },
                    {
                        mapKey: event_writer_id,
                        type: AIChatQSDataTypeEnum.STREAM,
                    },
                );
            } else {
        ...
        if (
            ...
            const showUI =
                getElements().findIndex(
                    (item) =>
                        item.token === call_tool_id &&
                        item.type === AIChatQSDataTypeEnum.TOOL_RESULT,
                ) >= 0;
            // 这里是直接使用引用设置的值，所以不需要在使用setContentMap设置回去
        ...
        (res: AIOutputEvent, status: AIToolResult['tool']['status']) => {
            try {
                ...
                toolResult.data.tool.summary =
                    toolResult.data.tool.summary ||
                    DefaultToolResultSummary[status]?.wait ||
                    '';
                // 设置执行结果错误数据内容(std_xxx_stderr)
                const errorResult =
                    streamToToolResultError.current.get(call_tool_id);
                if (errorResult && errorResult.status === 'end') {
                    toolResult.data.tool.execError = errorResult.content;
                    // error数据先出但未存在对应的工具执行结果，工具结果出现后直接使用并删除map中的缓存数据
                    streamToToolResultError.current.delete(call_tool_id);
                    toolResultErrorUUIDToCallToolID.current.delete(
                        errorResult.uuid,
                    );
                }
                updateElements({
        ...
        try {
            ...
            toolResult.data.tool.summary =
                statusInfo === 'user_cancelled'
                    ...
                    : summaryContent ||
                      DefaultToolResultSummary[toolResult.data.tool.status]
                          ?.result ||
                      '';
            // 设置执行结果错误数据内容(std_xxx_stderr)
            const errorResult =
        ...
        try {
            ...
            if (chatData) {
                ...
                if (chatData.parentGroupKey) {
                    updateElements(
                        {
                            mapKey: chatData.parentGroupKey,
                            type: AIChatQSDataTypeEnum.STREAM_GROUP,
                        },
                ...
                toolResult.reference = (toolResult.reference || []).concat([
                    data,
                ]);
                updateElements({
                    mapKey: toolResult.id,
                ...
                const chatData: AIChatQSData = {
                    ...
                    data: {
                        ...
                        NodeIdVerbose:
                            res.NodeIdVerbose ||
                            convertNodeIdToVerbose(res.NodeId),
                    },
                    reference: [data],
                };
    ...
    const handleSetData = useMemoizedFn((res: AIOutputEvent) => {
        try {
            ...
            if (res.Type === 'thought') {
                const { thought } =
                    (JSON.parse(ipcContent) as AIAgentGrpcApi.AIChatThought) ||
                    {};
            ...
            // 问题一次性的结果输出
            if (res.Type === 'result') {
                const { result, after_stream } =
                    (JSON.parse(ipcContent) as AIAgentGrpcApi.AIChatResult) ||
                    {};
            ...
            if (res.Type === 'tool_call_decision') {
                const data = JSON.parse(
                    ipcContent,
                ) as AIAgentGrpcApi.ToolCallDecision;
