...
const AIAgentLog: FC = () => {
    ...
    const onMessage = useMemoizedFn((data: string) => {
        try {
            const logInfo: MessageItem = JSON.parse(data)
            if (logInfo.level && logInfo.message && logInfo.timestamp) {
                setLogs((prev) => [...prev, logInfo])
            }
        } catch (error) { }
    });

    useBroadcastComm<string>(AIAgentLogChannelName, { onMessage });
