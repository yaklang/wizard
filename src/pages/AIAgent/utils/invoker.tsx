 ...
 * @returns
 */
export const isPluginExecuteLogFileItem = (fileItem: any) => {
    return Object.hasOwn(fileItem, 'action');
};

/** 根据权限码输出权限内容 */
export const modeToPermissions = (mode: string) => {
    // 确保输入八进制数
    if (!/^0[0-7]+$/.test(mode)) {
        return null;
    }
    ...
    try {
        switch (action) {
            case FileActionEnum.Read_Action: {
                const read =
                    action_message as PluginExecuteLogFile.ReadFileActionMessage;
            ...
            case FileActionEnum.Write_Action: {
                const write =
                    action_message as PluginExecuteLogFile.WriteFileActionMessage;
            ...
            case FileActionEnum.Create_Action: {
                const create =
                    action_message as PluginExecuteLogFile.CreateFileActionMessage;
                actionText = '创建';
                color = 'success';
            ...
            case FileActionEnum.Delete_Action: {
                const remove =
                    action_message as PluginExecuteLogFile.DELETEFileActionMessage;
                actionText = '删除';
                color = 'danger';
            ...
            case FileActionEnum.Status_Action: {
                const status =
                    action_message as PluginExecuteLogFile.STATUSFileActionMessage;
                actionText = '查看元信息';
                content = (
            ...
            case FileActionEnum.Chmod_Action: {
                const chmod =
                    action_message as PluginExecuteLogFile.CHMODFileActionMessage;
                actionText = '修改权限';
            ...
            case FileActionEnum.Find_Action: {
                const find =
                    action_message as PluginExecuteLogFile.FINDFileActionMessage;
                actionText = '查找';
