...
interface PageInfoStoreProps {
  ...
  setPageNodeInfoByPageGroupId: (key: any, gId: string, list: PageNodeItemProps[]) => void
  ...
  subscribeWithSelector(
    persist(
      (set, get) => ({
        ...
        updateGroupExpandOrRetract: (key) => {
          const newPages = get().pages
          const currentPage = newPages.get(key) || cloneDeep(defPage)
...
/** 处理WF需要缓存的数据 */
export const getFuzzerProcessedCacheData = (pageList: any[]) => {
