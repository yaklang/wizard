import type { AIMentionCommandParams } from '../ai-agent/components/aiMilkdownInput/aiMilkdownMention/aiMentionPlugin';
import { YakitRoute } from '../enums/yakitRoute';
import type { configManagementTabType } from '../types/interface/pageInfo';
...
interface PageInfoStoreProps {
    ...
    getCurrentGroupAllTabName: (key: any) => string[];
    /** 获取当前选中组 */
    getCurrentSelectGroup: (key: any) => PageNodeItemProps | undefined;
    clearAllData: () => void;
        ...
        persist(
            (set, get) => ({
                ...
                setPageNodeInfoByPageGroupId: (key, groupId, list) => {
                    ...
                    const groupList: PageNodeItemProps[] =
                        current.pageList.filter(
                            (ele) => ele.pageGroupId !== groupId,
                        );
                ...
                getPagesDataByGroupId: (key, groupId) => {
                    ...
                    return current.pageList.filter(
                        (ele) => ele.pageGroupId === groupId,
                    );
                },
                ...
                getPageInfoByRuntimeId: (routeKey, runtimeId) => {
                    ...
                    return current.pageList.find(
                        (ele) =>
                            ele?.pageParamsInfo[
                                pageInfoToRuntimeIdMap[routeKey]
                            ]?.runtimeId === runtimeId,
                    );
                },
                ...
                updatePagesDataCacheById: (key, value) => {
                    ...
                    const current: PageProps =
                        pages.get(key) || cloneDeep(defPage);
                    let updateIndex: number = current.pageList.findIndex(
                        (ele) => ele.pageId === value.pageId,
                    );
                    if (updateIndex !== -1) {
                ...
                removePagesDataCacheById: (key, id) => {
                    const newVal = get().pages;
                    const current: PageProps =
                        newVal.get(key) || cloneDeep(defPage);
                    const newPageList = current.pageList.filter(
                        (ele) => ele.pageId !== id,
                    );
                    newVal.set(key, {
                        ...current,
                ...
                getCurrentSelectGroup: (key) => {
                    ...
                    const currentPages: PageProps =
                        pages.get(key) || cloneDeep(defPage);
                    return currentPages.pageList.find(
                        (ele) => ele.pageGroupId === currentGroupId,
                    );
                },
                clearAllData: () => {
                    set({
            ...
            }),
            {
                ...
                storage: {
                    ...
                    setItem: async (
                        name,
                        value: StorageValue<PageInfoStoreProps>,
                    ) => {
                        const str = JSON.stringify({
                            state: {
                                ...value.state,
                                pages: Array.from(value.state.pages.entries()),
                                selectGroupId: Array.from(
                                    value.state.selectGroupId.entries(),
                                ),
                            },
                        });
...
export const getFuzzerProcessedCacheData = (pageList) => {
    const cache = pageList.map((ele) => {
        const advancedConfigValue =
            ele.pageParamsInfo?.webFuzzerPageInfo?.advancedConfigValue ||
            defaultAdvancedConfigValue;
        const hotPatchCode =
            ele.pageParamsInfo?.webFuzzerPageInfo?.hotPatchCode;
        return {
            groupChildren: [],
            ...
            pageParams: {
                ...
                request:
                    ele.pageParamsInfo?.webFuzzerPageInfo?.request ||
                    defaultPostTemplate,
