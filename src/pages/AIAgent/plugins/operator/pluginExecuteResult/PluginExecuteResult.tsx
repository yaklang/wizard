...
export const PluginExecuteResult: React.FC<PluginExecuteResultProps> =
    React.memo((props) => {
        const {
            streamInfo,
            runtimeId,
                ...
                switch (ele.type) {
                    ...
                    case 'http':
                        return runtimeId ? (
                            <PluginExecuteHttpFlow
                                runtimeId={runtimeId}
                                website={
                                    !!streamInfo.tabsInfoState['website']
                                        ?.targets
                                }
                                isCrawler={isCrawler}
                            />
                    ...
                    case 'log':
                        return (
                            <PluginExecuteLog
                                loading={loading}
                                messageList={streamInfo.logState}
                            />
                        );
                    case 'console': {
        ...
        const showTabs = useMemo(() => {
            if (
                !tempTotal &&
                !streamInfo.tabsState.find((item) => item.type === 'ssa-risk')
            ) {
                return streamInfo.tabsState.filter(
                    (item) => item.tabName !== '漏洞与风险',
                );
            }
        ...
        const tabBarRender = useMemoizedFn(
            (tab: HoldGRPCStreamProps.InfoTab, length: number) => {
                if (tab.type === 'risk') {
                    return (
                        <>
                            {tab.tabName}
                            <span
                                className={
                                    styles['plugin-execute-result-tabBar']
                                }
                            >
                                {length}
                            </span>
        ...
        const cardState = useCreation(() => {
            return streamInfo.cardState.filter(
                (item) => item.tag !== 'no display',
            );
                    ...
                    >
                        {showTabs.map((ele) => (
                            <TabPane
                                ...
                                key={ele.tabName}
                                className={
                                    styles['plugin-execute-result-tabPane']
                                }
                            >
        ...
        return (
            <div className={styles['plugin-execute-result-tab-content']}>
                {(title || extra) && (
                    <div
                        className={
                            styles['plugin-execute-result-tab-content-head']
                        }
                    >
                        <div>{title}</div>
                        <div>{extra}</div>
                ...
                <div
                    className={classNames(
                        styles['plugin-execute-result-tab-content-body'],
                        className,
                    )}
                >
                    {children}
...
const PluginExecuteCustomTable: React.FC<PluginExecuteCustomTableProps> =
    React.memo((props) => {
        const {
        ...
        const queryData = useMemoizedFn(() => {
            try {
                ...
                const newDataTable =
                    sorterTable?.order === 'none'
                        ? list
                        : sorterFunction(list, sorterTable, '') || [];
                ...
                <PluginExecuteResultTabContent
                    ...
                    title={
                        <span className={styles['table-title']}>
                            Total
                            <span className={styles['table-title-number']}>
                                {data.length}
                            </span>
                        </span>
                    }
