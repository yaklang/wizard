...
export const RadialGraph: React.FC<GraphProps> = (g) => {
    ...
    const {nodes, edges} = g.data as { nodes: PalmRadialGraphNode[], edges: PalmRadialGraphEdge[] };
    ...
    useEffect(() => {
        ...
        if (!graph) {
            ...
            graph = new G6.Graph({
                container: tag, width: g.width || tag.offsetWidth, height: g.height || tag.offsetHeight,
                modes: {
                    default: [
                        ...
                        {
                            type: 'edge-tooltip',
                            formatText: function formatText(model: any, e: any) {
        ...
        graph.data({
            nodes: nodes?.map((e: PalmRadialGraphNode) => {
                ...
                if (e?.is_model_rect) {
                    ...
                    e.label.split("\n").map(item => {
                        ...
                        width = (item.length > width ? item.length : width) * 8
                    });
            ...
            edges: edges?.map((e: PalmRadialGraphEdge) => {
