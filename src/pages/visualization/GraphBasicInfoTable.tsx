...
export const GraphBasicInfoTable: React.FC<GraphBasicInfoTableProps> = (p) => {
    ...
    const columns: ColumnsType<Palm.GraphBasicInfo> = [
        ...
        {
            title: "Type", render: (r: Palm.GraphBasicInfo) =>
                <Tag color={"blue"} onClick={e => setParams({...params, types: r.type})}
                >{GraphToChineseName(r.type)}</Tag>
        },
        {title: "来源", render: (r: Palm.GraphBasicInfo) => r.source && <Tag>{r.source}</Tag>},
        {title: "创建时间", render: (r: Palm.GraphBasicInfo) => <Tag>{moment.unix(r.created_at).toISOString()}</Tag>},
        {title: "创建时间", render: (r: Palm.GraphBasicInfo) => <Tag>{moment.unix(r.updated_at).toISOString()}</Tag>},
        {
            title: "操作", render: (r: Palm.GraphBasicInfo) => {
                return <div>
                    <Button.Group size={"small"}>
                        <Button type={"primary"} onClick={createGraphViewByModal(r.id)}>查看图</Button>
                        <Popconfirm
                            title={"确认删除该图例吗？无法恢复"}
                            onConfirm={() => {
                                deleteGraphById(r.id, () => {
                                    Modal.success({content: `删除图例「${r.id}」成功`})
                                }, () => {
                                    update(1, limit)
                                })
                            }}
                        >
                            <Button type={"dashed"} danger={true}>删除图例</Button>
                        </Popconfirm>
                    </Button.Group>
                </div>
            }
    ...
    return <Spin spinning={loading}>
        {p.miniMode ? "" : <div style={{marginBottom: 13}}>
            <Form
                layout={"inline"}
                onSubmitCapture={e => {
                    e.preventDefault()
            ...
            >
                ...
                <InputItem
                    label={"按来源搜索"}
                    value={params.source}
                    setValue={source => setParams({...params, source})}
                />
                <ManyMultiSelectForString
                    label={"选择图例类型"}
                    data={[
                    ...
                    value={params.types}
                    setValue={types => setParams({...params, types})}
                />
                <InputTimeRange
                    label={"筛选时间范围"}
                    start={params.start_timestamp}
                    end={params.end_timestamp}
                    setStart={start_timestamp => setParams({...params, start_timestamp})}
                    setEnd={end_timestamp => setParams({...params, end_timestamp})}
                />
                <SelectOne label={"排序依据"} value={params.order_by} data={
                    [{text: "按创建时间", value: "created_at"}, {text: "按更新时间", value: "updated_at"},]
                } setValue={order_by => setParams({...params, order_by})}/>
                <SelectOne label={"顺序"} value={params.order} data={
                    [{text: "正序", value: "asc"}, {text: "倒序", value: "desc"},]
            ...
            <Tabs.TabPane tab={"预览"} key={"2"}>
                ...
                <Row gutter={20}>
                    {loading ? <div>
                        ...
                        {data && data.map(e => <>
                            <Col xs={24} span={24} xl={p.graphOneCol ? 24 : 12}
                                 style={{width: "100%", height: "100%", marginBottom: 15}}>
                                <div style={{}}>
                                    <Card
                                        title={e.name} bordered={true}
                                    >
                                        <Button.Group>
                                            <Button type={"primary"}
                                                    onClick={createGraphViewByModal(e.id)}>查看大图</Button>
                                            <Popconfirm
                                                title={"确认删除该图例吗？无法恢复"}
                                                onConfirm={() => {
                                                    deleteGraphById(e.id, () => {
                                                        Modal.success({content: `删除图例「${e.id}」成功`})
                                                    }, () => {
                                                        update(1, limit)
                                                    })
                                                }}
                                        ...
                                        <GraphViewer height={500} width={600} id={e.id}/>
                                        <br/>
