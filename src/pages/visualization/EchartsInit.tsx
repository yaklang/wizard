...
import "./EchartsInit.css";
import { useSize } from "ahooks";
interface StackedVerticalBarProps {
  content: any;
}
// 堆叠柱状图
export const StackedVerticalBar: React.FC<StackedVerticalBarProps> = (
  props
) => {
  const { content } = props;
...
export const HollowPie: React.FC<HollowPieProps> = (props) => {
    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: function (obj: any) {
            ...
            return `${name}：${((value * 100) / centerData[0].value).toFixed(
              0
            )}%`;
          },
          // "{b}: {d}%",
  ...
  useEffect(() => {
    ...
    optionRef.current.title.subtext = [
      `{text|${centerData[0].value}}{small|台}`,
    ];
    // @ts-ignore
    const myChart = echarts.init(chartRef.current);
    myChart.setOption(optionRef.current);
  ...
  return (
    <div className="echarts-box echarts-box-hollow-pie" ref={ref}>
      <div className="echart-item echart-item-hollow-pie" ref={chartRef}></div>
    </div>
  );
};
...
export const MultiPie: React.FC<MultiPieProps> = (props) => {
  ...
  return (
    <div className="echarts-box" ref={ref}>
      <div className="echart-item echart-item-multi-pie" ref={chartRef}></div>
    </div>
  );
};
          ...
          {details && (
            <div className="echart-detail">
              <div className="echart-detail-item">
                <div className="echart-detail-item-title">{details.name}</div>
        ...
        {dataSource.map((item: any) => {
          return (
            <div className="echarts-card-item">
              ...
              <div className="echarts-card-content">{item.value}</div>
            </div>
          );
        })}
      </div>
