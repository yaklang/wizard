...
import { detailHeaderGroupOptions, SeverityMapTag, targetRouteMap } from './compoments/utils'
...
const TaskDetail: FC = () => {
  ...
  const [tableFilter, setTableFilter] = useSafeState<Record<string, any> | undefined>({})

  const [headerGroupValue, setHeaderGroupValue] = useSafeState<TTaskDetailHeaderGroups>(1)
  ...
  const { data, runAsync, loading } = useRequest(
    async (id, task_id) => {
      ...
      const filterData: TDetailDatailOptions = combinationData.filter(
        (item) => item.value !== undefined && item.value !== 'undefined' && item.value !== null,
      )
  ...
  const { runAsync: assetsVulnsFilterrunAsync } = useRequest(
    async () => {
      ...
      const transformSeverityList = severity?.map((it) => {
        const label = SeverityMapTag.find((item) => item.key.includes(it.key ?? ''))?.name
        return {
          Verbose: label,
          Total: it.value,
          value: it.key,
  ...
  useEffect(() => {
    const targetScriptType = scriptTypeOption.filter((item) => item.value !== 'weakinfo').map((item) => item.value)
  ...
  const requestCallback = useDependentCallback(
    (params: Parameters<RequestFunction>['0'], filter: Parameters<RequestFunction>['1']) => {
          ...
          try {
            ...
            const targetProtColumns = ProtColumns().map((item) =>
              item.dataIndex === 'execute_node'
                ? {
                    ...item,
                    wizardColumnsOptions: record?.scanner?.map(
          ...
          try {
            ...
            await assetsVulnsFilterrunAsync()
              .then((filterData) => {
                ...
                const assetsVulnsColumns = AssetsVulnsColumns(targetFilterData)
  ...
  if (!isReady || loading) {
    return <Spin spinning={!isReady || loading} className="w-full h-full justify-center items-center flex" />
  }

  // 枚举 展示table 高级筛选抽屉值
  const tableFilterEnum = (type: 1 | 2 | 3) => {
    return match(type)
      .with(1, () => <AssetsProtsFilterDrawer task_id={record?.task_id} page={page} />)
      .with(2, () => <AssetsVulnsFilterDrawer task_id={record?.task_id} page={page} />)
      .with(3, () => <AssertsDataFilterDrawer task_id={record?.task_id} />)
      .exhaustive()
  }
  ...
  return (
    <div className="flex align-start h-full">
      ...
      />
      {routeList.includes(record?.script_type) && headerGroupValue === 0 ? (
        <TaskRoadmap
        ...
        <WizardTable
          ...
          tableHeader={{
            ...
            options: {
              dowloadFile:
                headerGroupValue === 0 || headerGroupValue === 4 || headerGroupValue === 5
                  ? undefined
                  : {
                      fileName: `${exprotFileName[headerGroupValue]} (` + dayjs().unix() + ').csv',
                      params: {
                        type: ExportRequestKey?.[headerGroupValue],
                        data: {
                          ...tableFilter,
                          limit: -1,
                          task_id: record?.task_id,
