...
import type { TGetAssertsDataResponse, TGetAssetsProtsResponse, TGetAssetsVulnsResponse } from '@/apis/taskDetail/types'
...
const ProtColumns = (filterData?: { taskNodeData: any[] }): CreateTableProps<TGetAssetsProtsResponse>['columns'] => [
  ...
  },
  {
    ...
    columnsHeaderFilterType: 'input',
    width: 120,
    render: (_, render) => (render?.port ? <Tag color="default">{render?.port}</Tag> : '-'),
  },
  {
    title: '协议',
    dataIndex: 'proto',
  ...
  },
  {
    ...
    width: 260,
    render: (_, record) => (record?.updated_at ? dayjs.unix(record?.updated_at).format('YYYY-MM-DD HH:ss') : '-'),
  },
]
...
const AssetsVulnsColumns = (filterData: {
  ...
  return [
    ...
    },
    {
      ...
      width: 120,
      render: (value) => {
        const title = SeverityMapTag.find((item) => item.key.includes(value || ''))
        return <YakitTag color={title?.tag as YakitTagColor}>{title ? title.name : value || '-'}</YakitTag>
      },
    },
    {
      title: 'IP',
...
const AssertsDataColumns: CreateTableProps<TGetAssertsDataResponse>['columns'] = [
  ...
  },
  {
    ...
    width: 120,
    render: (_, record) => {
      const target = survivalStatusList.find((it) => it.value === record?.state)
      return target ? <div className={`color-[${target?.color}]`}>{target?.label}</div> : '-'
    },
  },
  {
    title: '风险状态',
  ...
  },
  {
    title: (
      <div className="flex gap-2 items-center">
        ...
        <span className="color-[#D33A30] font-normal text-xs pr-2" style={{ borderRight: '1px solid #EAECF3' }}>
          严重
        </span>
        <span className="color-[#F6544A] font-normal text-xs pr-2" style={{ borderRight: '1px solid #EAECF3' }}>
          高
        </span>
        <span className="color-[#F28B44] font-normal text-xs pr-2" style={{ borderRight: '1px solid #EAECF3' }}>
          中
        </span>
        <span className="color-[#FFB660] font-normal text-xs">低</span>
      </div>
    ),
    dataIndex: 'id',
    width: 240,
    render: (_, render) => {
      return (
        <div className="flex gap-2 items-center">
          ...
          <span className="color-[#FFB660] font-normal text-xs">{render?.low ?? 0}</span>
        </div>
      )
    },
  },
  {
    ...
    width: 180,
    render: (value) => (value ? dayjs.unix(value).format('YYYY-MM-DD HH:mm') : '-'),
  },
]
...
export { ProtColumns, AssetsVulnsColumns, AssertsDataColumns, companyInfoColumns, domainInfoColumns }
