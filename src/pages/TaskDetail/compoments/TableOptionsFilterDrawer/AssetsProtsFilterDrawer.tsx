...
import { AssetsProtsFilterDataList, sortDataByList, targetTitle, updateAssetsProtsFilterDataList } from './data'
...
// 端口资产高级筛选
const AssetsProtsFilterDrawer: FC<{ task_id?: string; page: UsePageRef }> = ({ task_id, page }) => {
  const { data } = useRequest(async () => {
    ...
    // 调用函数更新数据
    const updatedData = updateAssetsProtsFilterDataList(list, targetAssetsProtsFilterDataList)
          ...
          return (
            <Collapse
              ...
              items={value?.keys.map((it) => {
                const type = it === 'ipList' ? 'cidr' : 'service_type'
                ...
                return {
                  ...
                  extra: (
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation()
                        const targetData = data?.items[it]
                        const getTableParams = page.getParams()?.filter

                        const filterCheckedList = {
                          cidr: getTableParams?.cidr?.filter(
                            (key?: string) => !targetData?.map((item) => item.value).includes(key),
                          ),
                          service_type: getTableParams?.service_type?.filter(
                            (key?: string) => !targetData?.map((item) => item.value).includes(key),
                          ),
                        }
                        page.editFilter(filterCheckedList)
                      }}
                    >
                      重置
                    </Button>
                  ),
                  label: targetTitle[it as 'group' | 'sever' | 'data' | 'webSever' | 'fingerprint' | 'ipList'],
                  children: (
                    <div className={classNames(styles[`assets_prots_${type}`])}>
                      <Item name={type}>
                        <AssetsProtsGroupTag data={value.items[it]} />
                      </Item>
                    </div>
                  ),
                }
