...
import { TTaskDetail } from '@/apis/taskDetail/types';
...
const detailDatailList: TDetailDatailList = [
    {
        name: '任务ID',
        value: '47.52.100.30',
        key: 'task_id',
    ...
    {
        name: 'enbale-cve-baseline',
        value: 'true',
        key: 'enbale-cve-baseline',
    },
    {
        name: 'plugins',
...
const TaskDetailSider: FC<TTaskDetailSiderProps> = ({ id, data }) => {
    ...
    const transformDetailMemo = useMemoizedFn(() => {
        const transformDetailData = detailDatailList.map((it) => {
            const key = data?.[it?.key];
            const resultValue =
                typeof key !== 'undefined' && key !== null ? key : '-';
            return {
                ...it,
                value: resultValue,
            };
        });
        return transformDetailData;
    });
