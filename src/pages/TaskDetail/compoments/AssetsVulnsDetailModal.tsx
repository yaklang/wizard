...
>(({ title }, ref) => {
  ...
  const [showType, setShowType] = useSafeState<'detail' | 'code' | 'history'>('detail')
  ...
  return (
    <WizardModal footer={null} width={850} modal={model} title={title}>
      <YakitRadioButtons
        ...
        options={getOptions}
      />
      {showType === 'detail' && <InfoDescribe info={info} />}
      {showType === 'code' && (
...
const InfoDescribe: React.FC<{ info?: TGetAssetsVulnsResponse }> = ({ info }) => {
  return (
    <div className="px-2 pb-2">
          ...
          <div className="flex items-start gap-2 mb-2 flex-col">
            <div className="flex gap-4 items-center">
              <img
                ...
                src={SeverityMapTag.find((item) => item.key.includes(info?.severity || ''))?.img}
              />
            ...
            <div className="ml-16 flex gap-4 items-center">
              <span className="text-xs font-normal flex items-center">
                ...
                <span className="color-[#85899E] ">
                  {' '}
                  {info?.url ? <Paragraph copyable>{info?.url}</Paragraph> : '-'}
                </span>
              </span>
              <span className="text-xs font-normal">
                ...
                <span className="color-[#85899E]">
                  ...
                  {info?.created_at && info?.created_at > 0
                    ? dayjs.unix(info?.created_at).format('YYYY-MM-DD HH:ss')
                    : '-'}
                </span>
              </span>
              <span className="text-xs font-normal">
                ...
                <span className="color-[#85899E]">
                  {' '}
                  {info?.created_at && info?.created_at > 0
                    ? dayjs.unix(info?.updated_at).format('YYYY-MM-DD HH:ss')
                    : '-'}
                </span>
              </span>
            </div>
      ...
      <Row>
        <Col className="bg-[#F0F1F3] min-h-10 py-3 px-4 border-[#EAECF3] border" span={3}>
          IP
        </Col>
        <Col className="bg-[#F8F8F8] min-h-10 py-3 px-4 border-[#EAECF3] border" span={5}>
      ...
      <Row>
        ...
        <Col className="bg-[#F8F8F8] min-h-10 py-3 px-4 border-[#EAECF3] border" span={5}>
          ...
          >
            {!info?.detail?.WaitingVerified ? '已验证' : '未验证'}
          </div>
        </Col>
      </Row>
      ...
      <Row>
        ...
        <Col className="bg-[#F8F8F8] min-h-10 py-3 px-4 border-[#EAECF3] border" span={21}>
      ...
      <Row>
        <Col className="bg-[#F0F1F3] py-3 px-4 border-[#EAECF3] flex items-center border" span={3}>
          详情
        </Col>
        <Col className="bg-[#F8F8F8] py-3 px-4 border-[#EAECF3] border" span={21}>
