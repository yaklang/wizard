...
import { Button, Checkbox, Form, Input, message, Popover, Collapse, Select, Switch } from 'antd'
import { getValueByType, ParamsToGroupByGroupName, buildParamFormItem } from './helpers'
import {
  PresetPorts,
...
import { postStorageTaskScript, postThreatAnalysisScriptInformation } from '@/apis/task'
import type {
...
const ModifyTaskScript: FC = () => {
  ...
  const [cliParams, setCliParams] = useSafeState<YakScriptParamFull[] | null>(null)
  ...
  const { loading, run } = useRequest(postStorageTaskScript, {
    ...
    onSuccess: async () => {
      ...
      navigate('/system-management/task-script')
    },
    onError: (err) => {
      console.error(err)
    },
  ...
  useEffect(() => {
    ...
    cliParams.forEach((p) => {
      ...
      const val = getValueByType(p.paramValue, (p.typeVerbose || '').toLowerCase())
      newVals[key] = val
    })
    ...
    <Form form={form} layout="vertical" className="h-full">
      ...
      <div className="flex h-[calc(100%-64px)] px-4 border border-t-[#E9EBED] border-t-solid">
        <div className="w-1/5 pt-4 overflow-y-auto h-full">
          ...
          <Item label="脚本类型" name="script_type" initialValue="yaklang">
            <Select
        ...
        <div className="w-3/5 border border-x-[#E9EBED] border-x-solid px-4 mx-4">
          <WizardAceEditor style={{ minHeight: '100%' }} value={scriptValue} onChange={setScriptValue} />
        </div>
            ...
            <div className="h-full flex flex-col">
              ...
              <div className="overflow-auto p-2 text-sm h-full">
                <Collapse
                  ...
                  items={ParamsToGroupByGroupName(cliParams || []).map((group: { group: string; data: any[] }) => ({
                    key: group.group,
                    label: <div className="font-medium">参数组: {group.group}</div>,
                    children:
                      group.data && group.data.length > 0 ? (
                        group.data.map((p: any) => buildParamFormItem(p))
                      ) : (
                        <div className="color-[#85899E]">暂无解析结果，点击“刷新参数”或编辑脚本以自动解析。</div>
                      ),
                  }))}
                />
              </div>
            ...
            <>
              ...
              <Item noStyle dependencies={['script_type']}>
                {({ setFieldValue, getFieldValue }) => {
                  ...
                  return (
                    <Item
                      label={scriptType === 'weakinfo' ? '关键词' : '扫描目标'}
                      name={scriptType === 'weakinfo' ? ['prompt_args', 'keyword'] : ['prompt_args', 'target']}
                      extra={
                        <div className="flex items-center font-normal text-xs color-[#85899E]">
                          ...
                          <ChunkUpload
                            ...
                            onChange={(fileName) => {
                              setFieldValue(
                                scriptType === 'weakinfo' ? ['prompt_args', 'keyword'] : ['prompt_args', 'target'],
                                fileName,
                              )
                              setFieldValue('param_files', generateUniqueId())
                            }}
                          >
                      ...
                      <>
                        <Item noStyle dependencies={[]}>
                          {({ setFieldValue }) => {
                            return (
                              <Item
                                name={['prompt_args', 'preset-protes']}
                                ...
                                <Checkbox.Group
                                  ...
                                  onChange={(checked) => {
                                    const portsValue = checked
                                      ...
                                      .join()
                                    setFieldValue(['prompt_args', 'ports'], portsValue)
                                  }}
                                />
                              </Item>
                            )
                            ...
                            >
                              <Input.TextArea
                                ...
                                onChange={(e) => {
                                  ...
                                  setFieldValue(['prompt_args', 'preset-protes'], presetProtesKeysFromPorts(value))
                                  return value
                                }}
                              />
                            </Item>
                        ...
                        <Item
                          ...
                          name={['prompt_args', 'enable-cve-baseline']}
                          initialValue={true}
                        >
                          <Switch />
                        </Item>
