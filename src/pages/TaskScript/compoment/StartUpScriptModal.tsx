...
import { getNodeList, getTaskRun, getTaskStream, postEditScriptTask, postTaskStart } from '@/apis/task'
import type { StopOnRunTaskRequest, TaskListRequest, YakScriptParamFull } from '@/apis/task/types'
...
import { buildParamFormItem, getValueByType, ParamsToGroupByGroupName } from '../taskScript/helpers'
...
>(({ title, pageLoad, localRefrech, record, refreshAsync }, ref) => {
  ...
  const [scriptGroupList, setScriptGroupList] = useSafeState<ScriptGroupOption[]>([])
  const [keywordPlaceholder, setKeywordPlaceholder] = useSafeState('')
  const [editObj, setEditObj] = useSafeState<StopOnRunTaskRequest>({
    task_id: 0,
    task_type: 0,
  })
  const [scriptParameters, setScriptParameters] = useSafeState<YakScriptParamFull[]>([])
  ...
  const { data: scannerDataList, runAsync } = useRequest(
    async () => {
      ...
      const targetNodeList = list?.map((it) => ({
        ...
        date: it?.updated_at ? dayjs(new Date().getTime()).unix() - it.updated_at : '-',
      }))
  ...
  // 创建任务 请求
  const { runAsync: AddTaskRunAsync, loading: addLoading } = useRequest(postTaskStart, {
    manual: true,
  ...
  useImperativeHandle(ref, () => ({
    async open(items: TaskScriptListItem, groupOptions: ScriptGroupOption[]) {
      await runAsync()
        .then(() => {
          const parameterList = Array.isArray(items?.parameter) ? items.parameter : []
          const parameterDefaults = parameterList.reduce<Record<string, any>>((acc, param) => {
            const key = param.paramName
            if (!key) return acc
            acc[key] = getValueByType(param.paramValue, (param.typeVerbose || '').toLowerCase())
            return acc
          }, {})
          ...
          const normalizedPlugins = Array.isArray(mergedParams.plugins)
          ...
          const hasIpList = Array.isArray(items?.ip_list) ? items.ip_list.length > 0 : false

          const normalizedParams = {
            ...mergedParams,
            target: hasIpList ? (items?.ip_list ?? []).join(',') : mergedParams.target || mergedParams.keyword,
            'enable-cve-baseline':
              typeof mergedParams['enable-cve-baseline'] === 'boolean' ? mergedParams['enable-cve-baseline'] : true,
            ...
            'enable-web-login-brute':
              typeof mergedParams['enable-web-login-brute'] === 'boolean'
                ...
                : ['company_scan', 'login_brute_scan'].includes(items?.script_type ?? ''),
          ...
          const targetSetFormData = {
            ...
            ...items,
            execution_date:
              items?.sched_type === 2 && items?.start_timestamp ? dayjs.unix(items.start_timestamp) : undefined,
            timestamp:
              items?.sched_type === 3 && items?.start_timestamp && items?.end_timestamp
                ? [dayjs.unix(items?.start_timestamp), dayjs.unix(items?.end_timestamp)]
                : undefined,
        ...
        children: (
          <div className="space-y-4">
            {groupedParams.map((group) => {
              ...
              return (
                <div key={groupKey}>
                  <div className="mb-2 font-medium">参数组: {groupKey}</div>
                  {group.data && group.data.length > 0 ? (
                    group.data.map((param) => buildParamFormItem(param, ['params'], groupKey))
                  ) : (
                    <div className="text-xs text-[#85899E]">暂无解析结果，检查脚本参数配置。</div>
                  )}
                </div>
              )
            })}
  ...
  const onOk = async () => {
    try {
      ...
      const errorFields = err?.errorFields?.map((it: any) => it?.errors?.join(','))?.join(',')
      if (errorFields) {
        message.destroy()
    ...
    <WizardModal
      footer={
        <>
          ...
          <Button key="submit" type="primary" onClick={() => onOk()} loading={addLoading || editLoading || loading}>
            确定
          </Button>
        </>
      }
    ...
    >
      <div className="pb-2 px-6 overflow-auto max-h-[65vh]">
        <Form form={form} layout="horizontal">
          <Collapse
            key={scriptParameters?.length ? 'with-params' : 'no-params'}
            defaultActiveKey={scriptParameters?.length ? ['1', '2', 'script-params'] : ['1', '2']}
            bordered={true}
            ghost
            items={collapseItems}
          />
