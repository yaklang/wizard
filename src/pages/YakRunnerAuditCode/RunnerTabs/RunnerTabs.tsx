...
import { getWordWithPointAtPosition, type YaklangLanguageFindResponse } from '@/utils/monacoSpec/yakCompletionSchema'
...
import type { YakitIMonacoEditor, YakitITextModel } from '@/compoments/YakitUI/YakitEditor/YakitEditorType'
          ...
          itemIn.files.forEach((file) => {
            ...
            if (
              ...
              (!editorInfo ||
                ...
                (editorInfo && JSON.stringify(editorInfo.highLightRange) !== JSON.stringify(file.highLightRange)))
            ) {
              // 更新编辑器展示项
    ...
    useMemoizedFn((model: YakitITextModel) => {
      if (activeFile?.syntaxCheck) {
        const markers = activeFile.syntaxCheck
          ...
          .filter((item: any) => item !== null) as IMonacoEditorMarker[]
