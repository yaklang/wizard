...
import { useGetState, useInterval, useMemoizedFn, useSize, useUpdateEffect } from 'ahooks'
        ...
        {isBugFun(info) && (
          <div
            className={classNames(styles['bug'], {
              [styles['active-bug']]: info.Extra.filter((item: any) => item.Key === 'risk_hash').length > 0,
            })}
            onClick={(e) => {
              e.stopPropagation()
              goBUGDetail(info)
...
export const AuditCode: React.FC<AuditCodeProps> = (props) => {
  ...
  const initAuditTree = useMemoizedFn((ids: string[], depth: number) => {
    return ids.map((id) => {
      ...
      if (itemDetail.ResourceType === 'variable' || itemDetail.ResourceType === TopId) {
        ...
        // 数量为0时不展开 message除外
        if (parseInt(obj.Size.toString(), 10) === 0 && itemDetail.ResourceType !== TopId) {
          obj.isLeaf = true
        } else {
          obj.isLeaf = false
        }
        ...
        try {
          ...
          if (childArr.length > 0) {
            ...
            if (result) {
              ...
              result.Resources.filter((item: any) => item.VerboseType !== 'result_id').forEach(
              ...
              let isEnd = !!result.Resources.find((item: any) => item.VerboseType === 'result_id')
              // 如若请求数据未全部拿完 则显示加载更多
              if (!isEnd) {
                const newId = `${id}/load`
                setMapAuditDetail(newId, {
        ...
        if (result) {
          ...
          result.Resources.filter((item: any) => item.VerboseType !== 'result_id').forEach(
            (item: any, index: number) => {
              const { ResourceType, VerboseType, ResourceName, Size, Extra } = item
              let value = `${index}`
              const obj = Extra.find((item: any) => item.Key === 'index')
              if (obj) {
                value = obj.Value
              }
  ...
  const onAuditRuleSubmitFun = useMemoizedFn(async (textArea = '', Query?: { Key: string; Value: number }[]) => {
    try {
      resetMap()
      ...
      if (result && result.Resources.length > 0) {
        ...
        let variableIds: string[] = []
        // 构造树结构
        result.Resources.filter((item: any) => item.VerboseType !== 'result_id').forEach((item: any, index: number) => {
  ...
  useEffect(() => {
    const progress = Math.floor((streamInfo.progressState.map((item: any) => item.progress)[0] || 0) * 100) / 100
  ...
  const onAuditStreamRuleSubmitFun = useMemoizedFn(async () => {
    // if (!textArea) {
    //     warn('请输入规则');
    //     return;
    // }
