...
export const SplitView: React.FC<SplitViewProp> = memo((props) => {
    ...
    const [isVer] = useState<boolean>(isVertical || false);
    ...
    useEffect(() => {
        return () => {
            ...
            if (wrapperRef.current)
                wrapperResizeObserver.unobserve(wrapperRef.current);
        };
    }, []);
    /** ---------- 监听组件整体尺寸大小变化 End ---------- */
    ...
    const onSashMouseUp = useMemoizedFn(() => {
        setIsDown(false);
        indexSash.current = -1;
        startOffset.current = { x: -1, y: -1 };
        alreadyOffset.current = 0;
    ...
    const positionMoveing = useThrottleFn(
        useMemoizedFn((index: number, e: MouseEvent) => {
            ...
            const offset = isVer
                ? clientY - startOffset.current.y
                : clientX - startOffset.current.x;
    ...
    return (
        <div
            ref={wrapperRef}
            data-split-view-id={wrapperId}
            className={styles['split-view']}
        >
            {/* 位移线 */}
            {!isLastHidden && (
                <div
                    className={classNames(styles['sash-container'], {
                        [styles['hover']]: !isOver,
                    })}
                >
                    {elements.map((_, index) => {
                        ...
                        return (
                            <div
                                ...
                                key={
                                    sashIDs.current[index] ||
                                    `sash-${wrapperId}-${index}`
                                }
                                className={classNames(
                                    styles['split-view-sash'],
                                    {
