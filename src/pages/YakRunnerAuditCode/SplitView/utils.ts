 ...
 */
export const generateKnownSplitSize: (
    params: GenerateKnownSplitSizeParams,
) => SplitViewPositionProp[] = ({
    isVertical,
    wrapperLong,
    minLong,
    length,
    ...
    const defaultSize: SplitViewPositionProp = {
        width: 0,
        height: 0,
        left: 0,
        top: 0,
 ...
 */
export const resizedSplitSize: (
    params: ResizedSplitSizeParams,
) => SplitViewPositionProp[] | null = ({
    isVertical,
    resizeLong,
    minLong,
    positions,
    ...
    const oldViewLong = ps.reduce(
        (prev, current) => prev + (isVertical ? current.height : current.width),
        0,
    );
 ...
 * 里面的1代表分割线的占位长度-1px
 */
export const calculateOffsetRange: (
    params: CalculateOffsetRangeParams,
) => number[] = ({ length, sashIndex, rect, isVertical, minLong }) => {
 ...
 */
export const offsetSplitPosition: (
    params: OffsetSplitPositionParams,
) => SplitViewPositionProp[] | null = ({
    isVertical,
    minLong,
    positions,
    ...
    try {
        if (isleft) {
            ...
            // 计算允许的最大偏移量
            const maxOffset =
                compress.reduce((prev, current) => prev + current[longKey], 0) -
                compress.length * minLong;
