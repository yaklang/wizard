...
export const YakRunnerAuditCode: React.FC<YakRunnerAuditCodeProps> = (props) => {
  ...
  const onInitTreeFun = useMemoizedFn(async (rootPath: string, isFirst = true) => {
    try {
      resetMap(isFirst)
  ...
  const onOpenFileByPathFun = useMemoizedFn(async (data) => {
    try {
      ...
      if (file) {
        ...
        const { isPlainText } = await getCodeSizeByPath(path, fileSourceType)
  ...
  const YakRunnerContextProviderValue = useMemo(() => ({ store, dispatcher }), [store, dispatcher])
