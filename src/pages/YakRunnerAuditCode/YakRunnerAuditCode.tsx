...
import type {
    ...
    ShowItemType,
} from './BottomEditorDetails/BottomEditorDetailsType';
...
export const YakRunnerAuditCode: React.FC<YakRunnerAuditCodeProps> = (
    props,
) => {
    const { auditCodePageInfo } = props;
    // 页面数据
    const [pageInfo, setPageInfo] = useState<
        AuditCodePageInfoProps | undefined
    >(auditCodePageInfo);
    /** ---------- 文件树 ---------- */
    const [fileTree, setFileTree] = useState<FileTreeListProps[]>([]);
    /** ---------- 审计树 ---------- */
    const [projectName, setProjectName] = useState<string | undefined>(
        auditCodePageInfo?.Location,
    );
    ...
    const setAuditCodePageInfo = useMemoizedFn((auditCodePageInfo: string) => {
        try {
            ...
            const newPageInfo: AuditCodePageInfoProps =
                JSON.parse(auditCodePageInfo);
    ...
    useEffect(() => {
        ...
        if (pageInfo?.CodeRange) {
            try {
                const code_range: CodeRangeProps = JSON.parse(
                    pageInfo.CodeRange,
                );
                const { url, start_column, start_line, end_column, end_line } =
                    code_range;
    ...
    const onInitTreeFun = useMemoizedFn(
        async (rootPath: string, isFirst = true) => {
            try {
                resetMap(isFirst);
    ...
    const onResetAuditStatusFun = useMemoizedFn(async () => {
        try {
            ...
            const newAreaInfo = removeAuditCodeAreaFilesInfo(
                areaInfo,
                auditPaths,
            );
    ...
    const handleFetchFileList = useMemoizedFn(
        (path: string, callback?: (value: FileNodeMapProps[]) => any) => {
    ...
    const onOpenFileByPathFun = useMemoizedFn(async (data) => {
        try {
            const { params, isHistory, isOutside } = JSON.parse(
                data,
            ) as OpenFileByPathProps;
            ...
            if (file) {
                ...
                // 如若存在高亮显示 则注入
                if (highLightRange) {
                    cacheAreaInfo = updateAuditCodeAreaFileInfo(
                        areaInfo,
                        { ...file, highLightRange },
                        file.path,
                    );
                }
                const newAreaInfo = setAuditCodeAreaFileActive(
                    cacheAreaInfo,
                    path,
                );
                ...
                const { isPlainText } = await getCodeSizeByPath(
                    path,
                    fileSourceType,
                );
                ...
                const { newAreaInfo, newActiveFile } = addAuditCodeAreaFileInfo(
                    areaInfo,
                    scratchFile,
                    activeFile,
                );
    ...
    // 最后焦点聚集编辑器输出
    const onFixedEditorDetails = useMemoizedFn(
            ...
            // 拖拽到另一层
                ...
                newAreaInfo.forEach((item, index) => {
                    item.elements.forEach((itemIn, indexIn) => {
                        ...
                        if (itemIn.id === source.droppableId) {
                            // 从数组中移除元素并获取拖拽元素
                            const [ele] = newAreaInfo[index].elements[
                                indexIn
                            ...
                            if (filesLength === 0) {
                                if (item.elements.length > 1) {
                                    newAreaInfo[index].elements = newAreaInfo[
                                        index
                                    ].elements.filter(
                ...
                if (element && indexArr.length > 0) {
                    ...
                    if ((element as FileDetailInfo)?.isActive) {
                        newAreaInfo[indexArr[0]].elements[indexArr[1]].files =
                            newAreaInfo[indexArr[0]].elements[
                                indexArr[1]
                            ].files.map((item) => ({
                                ...item,
                                isActive: false,
                    ...
                    // 新增拖拽项
                    newAreaInfo[indexArr[0]].elements[indexArr[1]].files.splice(
                        destination.index,
                        0,
                        element,
                    );
                    ...
                    elements={[
                        {
                            element: (
                                <SplitView
                                    isLastHidden={
                                        areaInfo.length > 0 &&
                                        areaInfo[0].elements.length === 1
                                    }
                                    elements={[
                                        {
                                            element: (
                                                <RunnerTabs
                                                    tabsId={getTabsId(0, 0)}
                                                />
                                            ),
                                        },
                                        {
    ...
    const [active, setActive] = useSafeState<LeftSideType>('audit');
    ...
    const YakRunnerContextProviderValue = useMemo(
        () => ({ store, dispatcher }),
        [store, dispatcher],
    );

    return (
        <YakRunnerContext.Provider value={YakRunnerContextProviderValue}>
            <div className={styles['audit-code']} id="audit-code" tabIndex={0}>
...
// 代码审计进度信息
export const AuditCodeStatusInfo: React.FC<AuditCodeStatusInfoProps> = (
    props,
) => {
    const {
        title,
                ...
                <div className={styles['hint-right-download']}>
                    ...
                    <div className={styles['download-progress']}>
                        <Progress
                            ...
                            percent={Math.floor(
                                (streamData.Progress || 0) * 100,
                            )}
                            showInfo={false}
                        />
                        <div className={styles['progress-title']}>
                            进度 {(streamData.Progress * 100).toFixed(2)}%
                        </div>
                    </div>
                    {showDownloadDetail && (
                    ...
                    <div className={styles['log-info']}>
                        {logInfo.map((item: any) => {
                            if (item.level === 'error') {
                                return (
                                    <div
                                        key={item.data}
                                        className={styles['log-item-error']}
                                    >
                                        {item.data}
                                    </div>
                                );
                            ...
                            if (item.level === 'text') {
                                return (
                                    <div
                                        key={item.data}
                                        className={styles['log-item-text']}
                                    >
                                        {item.data}
                                    </div>
                                );
