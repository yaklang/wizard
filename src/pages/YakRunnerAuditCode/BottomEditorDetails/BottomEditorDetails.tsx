...
import type { OpenFileByPathProps, QuerySSARisksResponse, SSARisk } from '../YakRunnerAuditCodeType'
import {
...
export const BottomEditorDetails: React.FC<BottomEditorDetailsProps> = (props) => {
  ...
  // 数组去重
  const filterItem = (arr: any) => arr.filter((item: any, index: number) => arr.indexOf(item) === index)
      ...
      <div className={styles['content-resize-second']}>
        <Descriptions bordered size="small" column={column} labelStyle={{ width: 120 }}>
          <Descriptions.Item label="类型">
            {(info?.RiskTypeVerbose || info.RiskType).replace(/NUCLEI-/g, '')}
          </Descriptions.Item>
