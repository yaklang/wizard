...
import type { ActiveProps, RiskTreeProps, RunnerFileTreeProps, SelectOptionsProps } from './RunnerFileTreeType'
import { YakitButton } from '@/compoments/YakitUI/YakitButton/YakitButton'
import { OutlinePositionIcon, OutlineRefreshIcon, OutlineSearchIcon, OutlineXIcon } from '@/assets/icon/outline'
...
export const RunnerFileTree: React.FC<RunnerFileTreeProps> = memo((props) => {
  ...
  const onLoadData = useMemoizedFn((node: FileNodeProps) => {
    // 删除最外层文件夹时无需加载
    if (node.parent === null) return Promise.reject(new Error('最外层文件夹无需加载'))
      ...
      <>
        ...
        {options.length > 1 && (
          <div className={styles['compare-select-box']}>
            ...
            >
              <span style={{ fontSize: 12, whiteSpace: 'nowrap' }}>{checkItem === '' ? '查看未处置' : '只看新增'}</span>
            </YakitCheckbox>
          </div>
        )}
      </>
...
export const RiskTree: React.FC<RiskTreeProps> = memo((props) => {
  ...
  // 漏洞树Map
  const [, { set: setRiskMap, get: getRiskMap, reset: resetRiskMap }] = useMap<string, FileNodeMapProps>()
  const [, { set: setRiskChildMap, get: getRiskChildMap, reset: resetRiskChildMap }] = useMap<string, string[]>()
  // 选中的文件或文件夹
  const [foucsedKey, setFoucsedKey] = React.useState<string>('')
  // 展开项控制
  const [expandedKeys, setExpandedKeys] = React.useState<string[]>([])
  ...
  const onLoadData = useMemoizedFn((node: FileNodeProps) => {
    // 删除最外层文件夹时无需加载
    if (node.parent === null || !projectName) return Promise.reject(new Error('最外层文件夹无需加载'))
    return onLoadRiskTree(node.path, projectName)
  })

  // 跳转至审计结果
  const onJump = useMemoizedFn((data: any) => {
    const variable = data.Extra.find((item: any) => item.Key === 'variable')?.Value
    const index = data.Extra.find((item: any) => item.Key === 'index')?.Value
    const result_id = data.Extra.find((item: any) => item.Key === 'result_id')?.Value
    ...
    (
      ...
      try {
        if (e.selected) {
          ...
          if (!isFolder && data) {
            onJump(data)
            const arr = data.Extra.filter((item: any) => item.Key === 'code_range')
            const hash = data.Extra.find((item: any) => item.Key === 'hash')?.Value
            if (arr.length > 0) {
