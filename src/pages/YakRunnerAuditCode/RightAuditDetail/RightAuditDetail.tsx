    ...
    const titleRender = (info: GraphInfoProps) => {
        ...
        return (
            <div className={styles['result-render']}>
                ...
                <Tooltip
                    placement="topLeft"
                    title={`${code_range.url}:${code_range.start_line}`}
                >
                    <div
                        className={classNames(
                            styles['url-box'],
    ...
    const renderItem = (info: GraphInfoProps) => {
        ...
        return (
            <YakCodemirror
                ...
                highLight={{
                    from: {
                        line: start_line - source_code_line,
                        ch: start_column,
                    }, // 开始位置
...
export const AuditResultBox: React.FC<AuditResultBoxProps> = (props) => {
    ...
    const onExpendRightPathFun = useMemoizedFn((value: string) => {
        try {
            ...
            const index = getMapResultDetail(data.title).findIndex(
...
export const FlowChartBox: React.FC<FlowChartBoxProps> = (props) => {
        ...
        if (id) {
            ...
            if (element) {
                ...
                for (const titleElement of titles) {
                    if (titleElement.textContent === 'n1') {
                        // 获取匹配的 <title> 元素的父元素
                        ...
                        if (parentElement) {
                            ...
                            // 查找该元素下的所有 ellipse 标签
                            const ellipses =
                                parentElement.getElementsByTagName('ellipse');
                            // 提取样式设置为辅助函数以减少嵌套
                            const setEllipseStyle = (ellipse: SVGElement) => {
                                ellipse.style.stroke = '#8863F7';
                                ellipse.style.fill = 'rgba(136, 99, 247, 0.10)';
                            };
                            // eslint-disable-next-line max-depth
                            for (const ellipse of ellipses) {
                                setEllipseStyle(ellipse);
                            }
                        }
                        break; // 找到匹配的元素后停止遍历
                    }
    ...
    const onChangeSvgStyle = useMemoizedFn((id?: string) => {
        if (
            ...
            styleNodeRef.current !== parentElementNodeRef.current
        ) {
    ...
    const onRefreshAuditDetailFun = useMemoizedFn((newNodeId?: string) => {
        if (newNodeId && svgBoxRef.current) {
            ...
            const targetElement = Array.from(
                svgBoxRef.current.getElementsByTagName('title'),
            ).find((el) => el.innerHTML === newNodeId);
            if (targetElement) {
                // 获取父元素的 id
    ...
    useEffect(() => {
        ...
        return () => {
            emiter.off(
                'onCodeAuditRefreshAuditDetail',
                onRefreshAuditDetailFun,
            );
        };
    }, []);
                ...
                <div className={styles['relative-box']}>
                    <div className={styles['absolute-box']}>
                        ...
                        <div className={styles['extra']}>
                            <YakitButton
                                type="text2"
                                icon={<OutlineZoominIcon />}
                                onClick={handleZoomIn}
                            />
                            <YakitButton
                                type="text2"
                                icon={<OutlineZoomoutIcon />}
                ...
                <div className={styles['root-detail']}>
                    ...
                    <div className={styles['content']}>
                        {contentInfo && (
                            <Tooltip
                                placement="topLeft"
                                title={contentInfo.code_range.url}
                            >
                                <div
                                    className={classNames(
                                        styles['url-box'],
                        ...
                        {contentInfo && (
                            <YakCodemirror
                                readOnly={true}
                                fileName={contentInfo?.code_range.url
                                    .split('/')
                                    .pop()}
                                value={contentInfo?.ir_code || ''}
                                firstLineNumber={
                                    contentInfo.code_range.source_code_line
                                }
                                highLight={{
                                    from: {
                                        line:
                                            contentInfo.code_range.start_line -
                                            contentInfo.code_range
                                                .source_code_line,
                                        ch: contentInfo.code_range.start_column,
                                    }, // 开始位置
                                    to: {
                                        line:
                                            contentInfo.code_range.end_line -
                                            contentInfo.code_range
                                                .source_code_line,
    ...
    const initData = useMemoizedFn(async (params: AuditEmiterYakUrlProps) => {
        try {
            ...
            if (result && result.Resources.length > 0) {
                result.Resources[0].Extra.forEach((item: any) => {
                    if (item.Key === 'graph') {
                        setGraph(item.Value);
                    }
                    if (item.Key === 'graph_info') {
                        try {
                            let graph_info: GraphInfoProps[] = JSON.parse(
                                item.Value,
                            );
