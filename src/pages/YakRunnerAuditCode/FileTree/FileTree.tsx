...
import { FolderDefault, FolderDefaultExpanded, KeyToIcon } from '../FileTree/icon'
...
export const FileTree: React.FC<FileTreeProps> = memo((props) => {
  ...
  // 数组去重
  const filter = (arr: any) => arr.filter((item: any, index: number) => arr.indexOf(item) === index)

  const onExpandedFileTreeFun = useMemoizedFn((path: string) => {
  ...
  const titleRender = useMemoizedFn((nodeData: any) => {
    return (
      <FileTreeNode
        info={nodeData}
    ...
    >
      ...
      <div className={styles['node-content']}>
        ...
        <div className={classNames(styles['content-body'], 'yakit-content-single-ellipsis')}>
          <div className={classNames(styles['name'], 'yakit-content-single-ellipsis')} title={info.name}>
            {info.name}
            {info.count && (
