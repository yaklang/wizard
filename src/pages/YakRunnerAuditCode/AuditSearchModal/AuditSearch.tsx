...
export const AuditSearchModal: React.FC<AuditSearchProps> = memo((props) => {
    ...
    const [activeInfo, setActiveInfo, getActiveInfo] =
        useGetState<AuditDetailItemProps>();
    // 当前页数
    ...
    const getData = useMemoizedFn(async (page: number, pageSize = 10) => {
        try {
            ...
            if (result) {
                const initAuditDetail = result.Resources.filter(
                ...
                const newAuditDetail =
                    page === 1
                        ? initAuditDetail
                        : auditDetail.concat(initAuditDetail);
                if (isEnd) {
                    setHasMore(false);
                }
                if (page === 1) {
    ...
    useUpdateEffect(() => {
        const startLog = streamInfo.logState.find(
    ...
    useEffect(() => {
        ...
        if (
            visible &&
            selectedSearchVal !== keywords &&
            selectedSearchVal.length !== 0
        ) {
    ...
    const yakURLData = useMemo(() => {
        try {
            const Index = activeInfo?.Extra.find(
                (item) => item.Key === 'index',
            )?.Value;
            const CodeRange = activeInfo?.Extra.find(
                (item) => item.Key === 'code_range',
            )?.Value;
            const CodeFragment = activeInfo?.Extra.find(
...
export const YakRiskCodemirror: React.FC<YakRiskCodemirrorProps> = React.memo(
    (props) => {
