...
import type { AuditSearchProps, ExtraSettingDataProps } from './AuditSearchModalType'
...
export const onSetSelectedSearchVal = (v = '') => {
  selectedSearchVal = v
}
...
export const AuditSearchModal: React.FC<AuditSearchProps> = memo((props) => {
  ...
  const getData = useMemoizedFn(async (page: number, pageSize = 10) => {
    try {
      ...
      if (result) {
        const initAuditDetail = result.Resources.filter((item: any) => item.VerboseType !== 'result_id').map(
          (item: any, index: number) => {
            const { ResourceType, VerboseType, ResourceName, Size, Extra } = item
            let value = `${index}`
            const arr = Extra.filter((item: any) => item.Key === 'index')
            if (arr.length > 0) {
              value = arr[0].Value
            }
            const newId = `/${value}`
  ...
  useUpdateEffect(() => {
    const startLog = streamInfo.logState.find((item: any) => item.level === 'json')
  ...
  const onSearch = useMemoizedFn((newKeywords = '') => {
    setActiveInfo(undefined)
    setAuditDetail([])
          ...
          <div className={styles['tabs-box']}>
            ...
            >
              {extraSettingData.map((item) => {
                return <YakitTabs.YakitTabPane tab={item.label} key={item.value} />
              })}
            </YakitTabs>
          </div>
        )}
        ...
        {executing ? (
          <div className={styles['progress-opt']}>
            <Progress
              ...
              percent={Math.floor((streamInfo.progressState.map((item: any) => item.progress)[0] || 0) * 100)}
            />
