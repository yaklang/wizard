...
export const CollapseList: <T>(props: CollapseListProp<T>) => ReactNode | null =
    memo((props) => {
        const {
            type = 'sideBar',
            panelKey,
        ...
        const containerClassName = useMemo(() => {
            if (type === 'sideBar')
                return styles['collapse-list-container-side-bar'];
                    ...
                    {list.map((item, index) => {
                        return (
                            <Panel
                                ...
                                key={
                                    (item as Record<string, any>)[onlyKey] ||
                                    `${panelKey || 'collapse-list'}-${index}`
                                }
                            >
                                <div
                                    className={classNames(
                                        styles['list-item-render'],
                                        {
                                            [styles[
                                                'list-item-render-sideBar'
                                            ]]: type === 'sideBar',
                                        },
                                    )}
                                >
...
export const HelpInfoList: React.FC<HelpInfoListProps> = memo((props) => {
    ...
    return (
        <div className={styles['help-info-list']}>
            ...
            />
            {getList.length === 0 && (
                <div className={styles['no-data']}>暂无帮助信息</div>
            )}
        </div>
    );
});
