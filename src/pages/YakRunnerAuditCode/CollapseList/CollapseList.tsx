import React, { type ReactElement, type ReactNode, memo, useEffect, useMemo, useState } from 'react'
...
export const CollapseList: <T>(props: CollapseListProp<T>) => ReactNode | null = memo((props) => {
  ...
  const expandIcon = useMemoizedFn((panelProps) => {
    const { isActive } = panelProps
    return (
      <OutlineChevronrightIcon
        className={classNames({
          'collapse-expand-arrow': !!isActive,
        })}
      />
    )
  })
        ...
        {list.map((item, index) => {
          return (
            <Panel
              ...
              key={(item as Record<string, any>)[onlyKey] || `${panelKey || 'collapse-list'}-${index}`}
            >
              <div
              ...
              >
                {type === 'output' && <div className={styles['render-tail']} />}
                {renderItem(item)}
              </div>
            </Panel>
          )
...
export const HelpInfoList: React.FC<HelpInfoListProps> = memo((props) => {
  ...
  // 编辑器实例
  const [helpEditor, setHelpEditor] = useState<YakitIMonacoEditor>()
    ...
    return (
      <div className={styles['file-info-show']}>
        ...
        <Tooltip title={activeFile?.path}>
          <div className={classNames(styles['sub-title'], 'yakit-single-line-ellipsis')}>{activeFile?.path}</div>
        </Tooltip>
        ...
        <>
          {definitionList.map((item, index) => (
            <div
              ...
              onClick={() => onJumpEditorDetailFun(item.range)}
              key={`definitionList-${index}`}
            >
              {item.lineContent}
            </div>
          ))}
    ...
    if (helpEditor) {
      ...
      if (model && position) {
        ...
        ipcRenderer
          ...
          } as unknown as YaklangLanguageSuggestionRequest)
  ...
  return (
    <div className={styles['help-info-list']}>
      <YakitResizeBox
        ...
        firstRatio="40%"
        firstNode={
          <CollapseList type="output" onlyKey="key" list={getList} titleRender={titleRender} renderItem={renderItem} />
        }
        secondMinSize={300}
