...
import { Alert, Button, Card, Form, Input, InputNumber, message, Radio, Select, Spin } from 'antd'
...
const NodeConfig: FC = () => {
  ...
  const buildHostCommand = (nodeName: string, goarch: 'amd64', maxParallel: number) => {
    return [
  ...
  const buildDockerCommand = (nodeName: string, goarch: 'amd64', maxParallel: number) => {
    return [
  ...
  const buildWindowsCommand = (nodeName: string, goarch: 'amd64', maxParallel: number) => {
    return [
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const requiredFile: 'yak' | 'docker-compose' = point_type === 'docker' ? 'docker-compose' : 'yak'
  ...
  return (
    <div className="p-6 space-y-4">
      <Card bordered={false} className="shadow-sm">
        <div className="text-xl font-semibold text-[#1f2a44]">节点安装与并发配置</div>
      ...
      <Card bordered={false} className="shadow-sm">
        <Form form={form} layout="vertical" className="max-w-4xl">
          <div className="text-base font-medium text-gray-800">节点基础信息</div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              <Select options={[{ label: 'amd64', value: 'amd64' }]} />
            </Item>
          </div>

          <div className="mt-8 pt-6 border-t border-gray-100">
            <div className="text-base font-medium text-gray-800">扫描并发配置</div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
            ...
            >
              <InputNumber min={1} max={128} precision={0} className="w-64 max-w-full" />
            </Item>
          </div>
                ...
                {runCode ? (
                  <>
                    <div className="mb-2 flex items-center justify-between">
                      <div className="font-medium text-gray-200 not-italic">执行命令</div>
                      <Button
                        type="link"
                        size="small"
                    ...
                    <pre className="m-0 whitespace-pre-wrap break-all">{runCode}</pre>
                  </>
                ) : (
                  <div className="text-sm text-gray-500">{'>_ 等待生成命令...'}</div>
                )}
              </div>
            </Spin>
          </div>
