...
const Login = () => {
  ...
  const [generateCaptchaFn, setGenerateCaptchaFn] = useSafeState<() => void | undefined>();
  ...
  const onFinish = async (values: FieldType): Promise<void> => {
    try {
      // 将获取到的验证码全部转为小写
      const toLowerCaseCaptcha = captcha.replace(/[A-Za-z]/g, (char) => char.toLowerCase());
      const verificationCode = values?.verificationCode;
      // 将输入验证码全转为小写
      const toLowerCaseVerificationCode =
        verificationCode && verificationCode.replace(/[A-Za-z]/g, (char) => char.toLowerCase());
      if (toLowerCaseCaptcha === toLowerCaseVerificationCode) {
        await login(values);
        await updatePower();
        navigate('/');
      ...
      <div className="flex h-full relative w-full z-1">
        <div className="relative pt-4 pl-4">
          <div className="flex items-center gap-1">
            ...
            <div className="font-YouSheBiaoTiHei text-[25px] font-normal color=[#31343F]">分布式平台</div>
          </div>
        </div>
        <Card
        ...
        >
          ...
          <Form form={form} onFinish={onFinish}>
            ...
            >
              <Input placeholder="请输入用户名字" className="h-[52px] rounded-[12px] bg-[#F8F8F8]" />
            </Form.Item>
            <Form.Item<FieldType>
              name="password"
              rules={[
            ...
            >
              <Input placeholder="请输入密码" className="h-[52px] rounded-[12px] bg-[#F8F8F8]" />
            </Form.Item>
            <div className="relative">
              ...
              >
                <Input placeholder="请输入验证码" className="h-[52px] rounded-[12px] bg-[#F8F8F8]" maxLength={4} />
              </Form.Item>
