...
const License: FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const { license: stateLicense } = location.state || {}
      if (stateLicense) {
        form.setFieldsValue({ fetch_license: stateLicense })
        return
      }
      ...
      try {
        ...
        if (cancelled) return
        if (licenseCode) {
          form.setFieldsValue({ fetch_license: licenseCode })
          return
        }
        ...
        if (!cancelled) {
          showErrorMessage(error, '获取 License 申请码失败')
        }
      }
    }
  ...
  return (
    <div className="w-full flex justify-center items-center overflow-auto">
      <div className="w-3/5 pt-12">
        <h1 className="flex justify-center items-center my-4">使用 License 注册您的产品</h1>
        <Form form={form} labelCol={{ span: 4 }} wrapperCol={{ span: 20 }}>
          <div className="border border-b-solid border-gray-200 mb-6 pb-4">
            ...
            >
              <Input.TextArea rows={6} disabled={true} placeholder="请输入" />
            </Item>
            <div className="pl-44">
              <Button type="link" className="pt-0" onClick={headCopy}>
                点此复制该 License 请求码
              </Button>
              <div className="pl-4 pt-2">在申请 license 时，请把这一串申请码给销售人员以便生成您专属的 License</div>
            </div>
          </div>

          <Item label="您的许可证" name="license" rules={[{ required: true, message: '请输入您的许可证' }]}>
            <Input.TextArea rows={16} placeholder="请输入" />
          </Item>

          <div className="w-full flex items-center justify-end">
            <Button loading={loading} className="my-4 w-[83.2%] py-7" type="primary" onClick={handSumit}>
              点此使用 License 激活您的产品
            </Button>
          </div>
        </Form>
