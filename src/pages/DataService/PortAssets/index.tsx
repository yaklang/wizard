...
import type { TDeleteAssetsPortsBody, TGetAssetsProtsResponse } from '@/apis/taskDetail/types'
...
import { buildDeleteAllBody } from './buildDeleteAllBody'
...
const PortAssets: FC = () => {
  ...
  const { data: taskNodeData } = useRequest(async () => {
    ...
    const resultData = Array.isArray(list) ? list.map((it) => ({ label: it, value: it })) : []
    return resultData
  })
  ...
  const headVulnerabilityScanning = () => {
    ...
    const ipList = dataSource.map((item) => (idsList?.includes(item.id) ? item.host : undefined)).filter((list) => list)
  ...
  const handleBatchDeletePorts = () => {
    ...
    const filter = page.getParams()?.filter as Record<string, any> | undefined
    modal.confirm({
      title: '批量删除',
      content: isAll
      ...
      async onOk() {
        if (isAll) {
          await runDeletePorts(
            buildDeleteAllBody({
              ...filter,
              service_type: filter?.service_type ? [filter?.service_type] : undefined,
  ...
  return (
    <>
          ...
          options: {
            ...
            trigger: (
              <div className="flex gap-2 items-center">
                <Button danger disabled={!hasDeleteSelection} onClick={handleBatchDeletePorts}>
                  批量删除
                </Button>
                <Button
                  type="primary"
      ...
      />
      <CreateTaskScriptModal ref={openCreateTaskModalRef} pageLoad={page.onLoad} />
    </>
  )
}
