...
const CveLoophole: FC = () => {
    ...
    },
    {
      ...
      render: (value, record) => {
        if (value) {
          ...
          const targetItem = SeverityMapTag.find((item) => item.key.includes(toLowerCaseValue))
          return (
            <div
              className={classNames(styles['cve-list-product-success'], {
                [styles['cve-list-product-warning']]: targetItem?.tag === 'warning',
                [styles['cve-list-product-info']]: targetItem?.tag === 'info',
                [styles['cve-list-product-danger']]: targetItem?.tag === 'danger',
                [styles['cve-list-product-serious']]: targetItem?.tag === 'serious',
              })}
            >
              <div className={classNames(styles['cve-list-severity'])}>{targetItem?.name}</div>
              <span className={classNames(styles['cve-list-baseCVSSv2Score'])}>{record.exploitability_score ?? 0}</span>
            </div>
          )
        } else {
          return '-'
  ...
  return (
    <div className="h-full">
            ...
            trigger: (
              <div className="flex items-center justify-center gap-4">
                <Space.Compact>
                  ...
                  />
                  <Input.Search
                    placeholder={options.find((it) => it.value === search.key)?.text}
                    onSearch={async (value) => {
                      const uselessKey = options.filter((it) => it.value !== search.key)[0].value
      ...
      />
      <CveAllUpdateModal ref={CveAllUpdateModalRef} refresh={page.refresh} />
      <CveDifferUpdateModal ref={CveDifferUpdateModalRef} refresh={page.refresh} />
    </div>
  )
}
