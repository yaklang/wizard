...
export function getCompletionItemKindByName(
    name: string,
): languages.CompletionItemKind {
    const itemKind =
        languages.CompletionItemKind[
            name as keyof typeof languages.CompletionItemKind
        ];
    return itemKind !== undefined
        ? itemKind
        : languages.CompletionItemKind.Snippet;
}

export const setYaklangBuildInMethodCompletion = (
    methods: MethodSuggestion[],
) => {
...
export const setYaklangCompletions = (data: CompletionTotal) => {
    ...
    completions = {
        libCompletions: data.libCompletions.filter(
            (i) => !i.libName.startsWith('_'),
        ),
        fieldsCompletions: [],
    ...
    const globalLibs = data.libCompletions.filter(
        (i) => i.libName === '__global__',
    );
...
export const getDefaultCompletions = (
    position: Position,
...
export const getDefaultCompletionsWithRange = (
    range: IRange,
): languages.CompletionItem[] => {
    return [...globalSuggestions, ...extraSuggestions].map((i) => {
        return {
...
const removeRepeatedSuggestions = (
    params: languages.CompletionItem[],
): languages.CompletionItem[] => {
...
const loadMethodFromCaller = (
    ...
    const pushCompletion = (
        i: MethodSuggestion,
        desc: MethodSuggestionItem,
    ) => {
        const labelDef = !!desc.DefinitionVerbose
            ? `${desc.DefinitionVerbose}`
            : `${desc.InsertText}`;
        items.push({
            insertTextRules:
                languages.CompletionItemInsertTextRule.InsertAsSnippet,
            ...
            label:
                i.Verbose === ''
                    ? `${labelDef}: ${desc.Description}`
                    : `${i.Verbose}.${labelDef}:  ${desc.Description}`,
            detail: `${i.Verbose}`,
            range: range,
    ...
    for (let i = 0; i < libCompletions.length; i++) {
        ...
        if (lastWord === currentLib.libName) {
            currentLib.functions.forEach((f) => {
                let compLabel = f.definitionStr.startsWith('func ')
                    ? f.definitionStr.substring(5)
                    : f.definitionStr;
            ...
            return {
                suggestions: [
                    ...items,
                    ...loadMethodFromCaller(lastWord, line, insertRange),
                ],
            };
        }
    }
...
export const getWordWithPointAtPosition = (
    ...
    if (lastChar === '.') {
        // 尝试往前找一个单词
        let lastWord = model.getWordAtPosition(
            new monaco.Position(position.lineNumber, iWord.startColumn - 2),
        );
        if (lastWord !== null) {
            iWord = {
