...
const McpModal = forwardRef<UseModalRefType, object>((_, ref) => {
  ...
  // 启动mcp
  const { runAsync: startMcp, loading: startMcpLoading } = useRequest(postStartMcpApi, {
    manual: true,
    onSuccess: () => {
      enabled ? message.error('停止成功') : message.success('启动成功')
    },
  })
  ...
  return (
    ...
    >
      <div className=" bg-white px-6 pb-6 pt-5 text-[#262626]">
        <div className={enabled ? 'mb-5 rounded bg-[#F6FFED] px-4 py-2.5' : 'mb-5 rounded bg-[#FFF1F0] px-4 py-2.5'}>
          <span className={enabled ? 'text-[#262626]' : 'text-gray-700'}>当前状态：</span>
          <span className={enabled ? 'font-medium text-[#389E0D]' : 'text-[#F5222D]'}>
            {enabled ? '已启用' : '未启用'}
          </span>
        </div>
        ...
        {enabled ? (
          <Tag color="blue">
            {sseStartupUrl} <CopyOutlined className="cursor-pointer  hover:text-[#1677FF]" onClick={copySseUrl} />
          </Tag>
        ) : (
          <div>
            <Input
