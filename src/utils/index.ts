...
export const formatSelectOptions = (
  ...
  try {
    return dataSource.map((item) => ({
      label: typeof fields.label === 'function' ? fields.label(item) : item[fields.label ?? 'text'],
      value: typeof fields.value === 'function' ? fields.value(item) : item[fields.value ?? 'value'],
    }))
  } catch (e) {
    console.error(e)
    return []
...
export const isNumber = (n: any, strict = true) => {
  const isStrictNumber =
    Object.prototype.toString.call(n) === '[object Number]' && !Number.isNaN(n) && Number.isFinite(n)
  if (strict) {
...
// 对比两个对象是否相同
const deepEqual = (pre: Record<any, any> = {}, cur: Record<any, any> = {}): boolean => {
  // 如果是同一个引用，直接相等
...
const createFlatTreeWithId = (parentId: any) => (tree: any) =>
  ...
  ].concat(...defaultTo([], tree.children).map(createFlatTreeWithId(tree.id)))
...
// routelist 节点key转换
const processMenu = (menus: RouteObjectRootMy[], navigate: (key: string) => void): ResultItem[] => {
...
const findPathNodes = (
  ...
  for (const route of routes) {
    ...
    const pathRegex = new RegExp(`^${currentPath.replace(/:\w+/g, '[^/]+')}$`)
    ...
    if (route.children) {
      const childResult = findPathNodes(targetPath, route.children, currentPath)
      if (childResult) {
        // 找到子节点路径，将当前节点添加到结果前
        return [{ name: route.name ?? '', path: currentPath }, ...childResult]
      }
    }
  }
