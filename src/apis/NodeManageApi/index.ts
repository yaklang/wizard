...
// 获取节点管理 表格数据
const getNodeManage = (params: QueryPalmNodeParams): Promise<ResponseData<TableResponseData<Palm.Node[]>>> =>
...
const deleteNodeManage = (
  ...
  axios.delete<never, ResponseData<TableResponseData<Palm.Node[]>>>(`/api/node`, {
    params,
  })
...
const postUpdateLocation = (data: {
  ...
  nickname: string
  node_id: string
}): Promise<ResponseData<boolean>> => axios.post<never, ResponseData<boolean>>('/api/node/update-location', data)

//  更新节点数据
const postNodesDownloadDataRun = (data: TPostNodesDownloadDataRunRequest): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>('/api/task/start/nodes-download-data/run', data)
...
const postHostAliveDetectionRun = (data: {
  ...
  hosts: string
  nodes_id: string[]
}): Promise<ResponseData<TableResponseData<PostHostAliveDetectionRunRequest>>> =>
  axios.post<never, ResponseData<TableResponseData<PostHostAliveDetectionRunRequest>>>(
    '/api/task/start/host-alive-detection/run',
    data,
  )
...
const getScannerObservabilityOverview = (params?: {
  ...
  axios.get<never, ResponseData<ScannerObservabilityOverview>>('/api/ssa/observability/scanner/overview', {
    params,
  })
