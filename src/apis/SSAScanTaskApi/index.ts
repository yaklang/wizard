...
const scanSSAProject = (
  ...
  axios.post<never, ResponseData<TSSATaskResponse>>(
    ...
    data || {},
    opts?.scan_mode && opts.scan_mode !== 'auto' ? { params: { scan_mode: opts.scan_mode } } : undefined,
  )

// GET /ssa/task - 查询任务列表
const querySSATasks = (params?: TSSATaskQueryParams): Promise<ResponseData<TSSATaskListResponse>> =>
...
const querySSAArtifactSummary = (taskId: string): Promise<ResponseData<TSSAArtifactMetricsSummary>> =>
  axios.get<never, ResponseData<TSSAArtifactMetricsSummary>>(`/api/ssa/task/${taskId}/artifact/summary`)

const querySSAArtifactEvents = (
  taskId: string,
  params?: { stage?: string; limit?: number },
): Promise<ResponseData<TSSAArtifactEventsResponse>> =>
  axios.get<never, ResponseData<TSSAArtifactEventsResponse>>(`/api/ssa/task/${taskId}/artifact/events`, { params })
