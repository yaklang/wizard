...
export const getSSARiskFilterOptions = (signal?: AbortSignal): Promise<ResponseData<TSSARiskFilterOptions>> =>
  axios.get<never, ResponseData<TSSARiskFilterOptions>>('/api/ssa/risk/filter-options', {
    signal,
  })
...
export const postSSARisk = (data: TSSARiskRequest): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>('/api/ssa/risk', data)

// POST /ssa/risk - 更新单个 SSA 风险（简化版，用于标记已读等）
export const updateSSARisk = (data: Partial<TSSARiskRequest> & { id: number }): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>('/api/ssa/risk', data)

// DELETE /ssa/risk - 删除 SSA 风险
export const deleteSSARisk = (params: { id?: number; hash?: string }): Promise<ResponseData<boolean>> =>
...
export const batchUpdateSSARisks = (data: TSSARiskBatchRequest): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>('/api/ssa/risk/batch', data)

// GET /ssa/risk/export - 导出 SSA 风险
export const exportSSARisks = (params?: TSSARiskExportParams): Promise<ResponseData<TSSARiskExportData>> =>
  axios.get<never, ResponseData<TSSARiskExportData>>('/api/ssa/risk/export', {
    params,
  })

// GET /ssa/risk/export - 下载 PDF 报告
export const exportSSARiskReportPDF = (params: TSSARiskExportParams): Promise<ResponseData<Blob>> =>
...
export const exportSSARiskReportDocx = (params: TSSARiskExportParams): Promise<ResponseData<Blob>> =>
  axios.get<never, ResponseData<Blob>>('/api/ssa/risk/export', {
    params: {
      ...params,
      format: 'docx',
...
export const importSSARisks = async (file: File): Promise<ResponseData<TSSARiskImportResult>> => {
  const formData = new FormData()
  formData.append('file', file)
  try {
    const response = await axios.post<never, ResponseData<TSSARiskImportResult>>('/api/ssa/risk/import', formData, {
      headers: {
...
export const getSsaRiskAuditFiles = (
  ...
  axios.get<never, ResponseData<TSSARiskRelatedFiles>>('/api/ssa/risk/audit/files', {
    params: { hash, task_id: taskId },
    signal,
  })
...
export const getSsaRiskFileContent = (
  ...
  axios.get<never, ResponseData<TSSARiskFileContent>>('/api/ssa/risk/audit/file/content', {
    params: { hash, task_id: taskId, file_path: filePath },
    signal,
  })

// POST /ssa/risk/disposal - 创建或更新 SSA 风险处置
export const postSSARiskDisposal = (data: TSSARiskDisposalRequest): Promise<ResponseData<boolean>> =>
...
export const getSSARiskDisposals = (
  ...
  axios.get<never, ResponseData<TSSARiskDisposalListResponse>>('/api/ssa/risk/disposal', {
    params,
    signal,
  })

// GET /ssa/risk/disposal/fetch - 获取单个 SSA 风险处置详情
export const getSSARiskDisposal = (id: number, signal?: AbortSignal): Promise<ResponseData<TSSARiskDisposal>> =>
  axios.get<never, ResponseData<TSSARiskDisposal>>('/api/ssa/risk/disposal/fetch', {
    params: { id },
    signal,
  })

// DELETE /ssa/risk/disposal - 删除 SSA 风险处置
export const deleteSSARiskDisposal = (id: number): Promise<ResponseData<boolean>> =>
