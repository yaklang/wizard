...
import type { ResponseData, TableRequestParam, TableResponseData } from '@/utils/commonTypes';
import type {
  AccountParams,
  AccountResponse,
...
export const accountList = (
  ...
  axios.post<never, ResponseData<TableResponseData<AccountResponse>>>('/sys/user/page', data);
...
// 用户详情
export const accountUserInfo = (id: TableRequestParam<string>): Promise<ResponseData<AccountUserInfoResponse>> =>
  axios.get<never, ResponseData<AccountUserInfoResponse>>(`/sys/user/info/${id}`);

// 用户添加
export const postUserSave = (data: UserSaveParmas): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>('/sys/user/save', data);

// 用户编辑
export const postUserEdit = (data: UserSaveParmas): Promise<ResponseData<boolean>> =>
