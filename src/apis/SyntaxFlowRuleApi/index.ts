...
const getSyntaxFlowRules = (
  ...
  axios.get<never, ResponseData<TSyntaxFlowRuleListResponse>>(`/api/syntaxflow/rule`, { params, signal })

// POST /syntaxflow/rule
const postSyntaxFlowRule = (data: TSyntaxFlowRuleRequest): Promise<ResponseData<boolean>> =>
  axios.post<never, ResponseData<boolean>>(`/api/syntaxflow/rule`, data)

// DELETE /syntaxflow/rule
const deleteSyntaxFlowRule = (params: { rule_name?: string; rule_id?: string }): Promise<ResponseData<boolean>> =>
...
const fetchSyntaxFlowRule = (
  ...
  axios.get<never, ResponseData<TSyntaxFlowRule>>(`/api/syntaxflow/rule/fetch`, {
    params,
    signal,
  })
...
const importSyntaxFlowRules = (
  ...
  axios.post<never, ResponseData<string>>(`/api/syntaxflow/rule/import`, data, {
    headers: {
...
const getAsyncTaskStatus = (taskId: string): Promise<ResponseData<TAsyncTaskStatusResponse>> =>
  axios.get<never, ResponseData<TAsyncTaskStatusResponse>>(`/api/async/task/status`, {
...
const createRuleSnapshot = (
  note?: string,
): Promise<ResponseData<{ version: string; hash: string; rule_count: number }>> =>
  axios.post<never, ResponseData<{ version: string; hash: string; rule_count: number }>>(
    `/api/syntaxflow/rules/snapshot`,
    { note },
  )
...
const updateSyntaxFlowRuleMetadata = (
  ...
  axios.put<never, ResponseData<TSyntaxFlowRule>>(`/api/syntaxflow/rule/update`, data)

// GET /syntaxflow/rule/filter-options - 获取规则筛选选项
const getSyntaxFlowRuleFilterOptions = (signal?: AbortSignal): Promise<ResponseData<TSyntaxFlowRuleFilterOptions>> =>
  axios.get<never, ResponseData<TSyntaxFlowRuleFilterOptions>>(`/api/syntaxflow/rule/filter-options`, { signal })
