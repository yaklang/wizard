...
// 插件日志条数
export const LIMIT_LOG_NUM_NAME = 'limit_log_num';
...
export const convertCardInfo = (
    maps: Map<string, HoldGRPCStreamProps.CacheCard>,
) => {
...
export interface HoldGRPCStreamParams {
    ...
    /** @name 额外的数据过滤方法 */
    dataFilter?: (
        obj: StreamResult.Message,
        content: StreamResult.Log,
    ) => boolean;
    /** @name 设置run-time-id值 */
...
export default function useHoldGRPCStream(params: HoldGRPCStreamParams) {
    ...
    const [streamInfo, setStreamInfo, getStreamInfo] =
        useGetState<HoldGRPCStreamInfo>({
            progressState: [],
            cardState: [],
            tabsState: [],
            tabsInfoState: {},
    ...
    // runtime-id
    const runTimeId = useRef<{ cache: string; sent: string }>({
        cache: '',
        sent: '',
    });
    // progress
    ...
    const handleResults = useMemoizedFn(() => {
        ...
        // card
        const cacheCard: HoldGRPCStreamProps.InfoCards[] = convertCardInfo(
            cardKVPair.current,
        );
        // tabs
